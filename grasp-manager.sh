...
core_segment_functions="csv classic classic_plot none"
core_current_functions="none"
core_tile_functions="csv none"
...
default_git_server_module_dependencies=git@code.grasp-open.com:open/module-dependencies.git
...
indentation_errors=$(awk 'NF > 0 && (!/^(    )*[a-zA-Z0-9#]/) {print NR}' grasp-manager.yml)

if [  $(echo $indentation_errors | wc -w) -gt 0 ]
then
    gmecho_title "GRASP-Manager error: The indentation in grasp-manager.yml is not correct. Indentation must be 4 white spaces. Tab or any other number of spaces is not allowed"
    echo "Following lines have wrong indentation: " $indentation_errors
    exit -1
fi
...
function help {
    ...
    gmecho "  install [environment]" ": Installs the code using 'environment' constants without synchronizing the code. If environment is not defined last used will be used (or default if it does not exist)"
...
function backup {    
    # Before backup anything, we check if needed to be backed up. If something need
    # to be saved, we ask for confirmation. If not, we go ahead.
    need_backup=0
    identified_problems=""
    ...
    then
        need_backup=1
        pwd_output=$(pwd)
        identified_problems=$(echo $identified_problems" - "$pwd_output"\n")
    fi
    ...
    then
        echo ""
        gmecho_title "Warning: Some files are not saved!"
        echo "GRASP-Manager does not work properly if the repository is not clean"
        echo "Specifically you have to check the changes you have done in following folders (repositories):"
        echo  -e ' '$identified_problems"If you want to avoid this issue in the future you have to have a 'clean' repository"
        echo "understanding that 'clean' means that when you type 'git status' it returns 'nothing to commit'"
        gmecho "Do you want GRASP-Manager undo your changes (backing them up) before continue? (y/N)"
        echo "Please, note that if you select 'yes', you may lose some progress"

        while true; do
            read yn
            case $yn in
                [Yy] ) break;;
                [Nn] ) gmecho "Aborting!" && exit -1;;
                * ) gmecho "Aborting! (Answer was not 'y' or 'n')" && exit -1;; #echo "Please answer yes or no.";;
            esac
        done        
    fi
...
function install_extensions {
    ...
    then
        ...
        do
            ...
            url_addr=$(echo ${repos_to_install_urls[$i]} | cut -d '+' -f1)
            url_reff=$(echo ${repos_to_install_urls[$i]} | cut -d '+' -f2)
           ...
           then
                ...
                do
                    ...
                    git clone $url_addr $repos_path/$repo_path
                    status_git=$?
                    let tries=$tries+1
                done 
                if [ "$tries" -ge "10" ]
           ...
           trap - 2
            ...
            # Move to fix tag if it was forced in the settings.
            if [ "$url_addr" != "$url_reff" -a "$url_reff" != "" ] 
            then 
                cd ${repos_to_install_paths[$i]}
                gmecho_title "WARNING: Moving ${repos_to_install_types[$i]} ${repos_to_install_names[$i]} to specific git tag ($url_reff)"
                git checkout  $url_reff
                cd - > /dev/null
            fi
        done
    fi
    ...
    then
        ...
        do
            ...
            then
                ...
                # Added checking for verifing if the repo is attached to specific commit
                url_stored=$(set | grep conf_${compilation_environment}[A-Za-z_]*${installed_repo_types[$i]}[s]*_${installed_repo_names[$i]}= | cut -f1 -d=)
                url=${!url_stored}
                # options for modules are different (true/false/ref)
                if [ ${installed_repo_types[$i]} != "module" ]
                then
                    url_addr=$(echo $url | cut -d '+' -f1)
                    url_reff=$(echo $url | cut -d '+' -f2)
                    if [ "$url_addr" == "$url_reff" ] 
                    then 
                        url_reff=''
                    fi
                else
                    if [ "$url" != "true" -a "$url" != "false" -a "$url" != "~" ] 
                    then
                        url_reff="$url"
                    else
                        url_reff=''
                ...
                if [ "$url_reff" != "" ] 
                then
                    extension_tag=$url_reff
                    gmecho_title "WARNING: Skipping ${installed_repo_types[$i]} ${installed_repo_names[$i]}. It cannot be moved to different reference because it is fixed in grasp-manager.yml"
                fi
...
function prepare_environment () {
    ...
    flag_name=conf_${compilation_environment}_compilation_python
    flag_value=${!flag_name}
    if [ "$flag_value" != "" -a "$flag_value" != "~" ]
    then
        compilation_python=$flag_value
        let counter=$counter+1
    fi
    ...
    then
        ...
        if [ "$modules_dependencies" != "~" -a "$modules_dependencies" != "true" ]
        then
            repos_to_install_urls+=($default_git_server_module_dependencies"+""$modules_dependencies")
        else
    ...
    then
        ...
        if [ "$modules_gpgpu" != "~" -a "$modules_gpgpu" != "true" ]
        then
            repos_to_install_urls+=($default_git_server_module_gpgpu"+""$modules_gpgpu")
        else
    ...
    then
        ...
        if [ "$modules_kernels_modifier" != "~" -a "$modules_kernels_modifier" != "true" ]
        then
            repos_to_install_urls+=($default_git_server_module_kernels_modifier"+""$modules_kernels_modifier")
        else
    ...
    then
        ...
        if [ "$modules_internal_examples" != "~" -a "$modules_internal_examples" != "true" ]
        then
            repos_to_install_urls+=($default_git_server_module_internal_examples"+""$modules_internal_examples")
        else
    ...
    then
        ...
        if [ "$modules_models" != "~" -a "$modules_models" != "true" ]
        then
            repos_to_install_urls+=($default_git_server_module_models"+""$modules_models")
        else
...
function checkout () {
    ...
    then
        ...
        gmecho_title "ERROR: $1 is not a valid git reference in current GRASP repository"
    fi
}

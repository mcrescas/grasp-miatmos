...
char* grasp_compilation_information_str(){
    char *str = (char*) malloc(sizeof(char) * 32768);
    char *tmp;
    char tmp_str[2024];

    sprintf(str, " GRASP core version: %s (commit: %s ; branch_name: %s)\n",GRASP_VERSION, GRASP_COMMIT_REF, GRASP_BRANCH_NAME);
    sprintf(tmp_str, " Compiled on %s commit of %s\n", GRASP_COMPILATION_DATE, GRASP_COMMIT_DATE);
    strcat(str, tmp_str);
    sprintf(tmp_str, " With C compiler: %s \n", GRASP_CC_VERSION);
    strcat(str, tmp_str);
    sprintf(tmp_str, " With FORTRAN compiler: %s \n", GRASP_FC_VERSION);
    strcat(str, tmp_str);
    sprintf(tmp_str, " Using %s constant set and build type %s\n", GRASP_CONSTANTS_SET, GRASP_BUILD_TYPE);
    strcat(str, tmp_str);
    sprintf(tmp_str, " Other options:");
    strcat(str, tmp_str);
    if(strcmp(GRASP_MPI,"yes")==0){
        sprintf(tmp_str, " using mpi,");
        strcat(str, tmp_str);
    }
    if(strcmp(GRASP_MODELS,"no")!=0){
        sprintf(tmp_str, " models module compiled (%s),", GRASP_MODELS);
        strcat(str, tmp_str);
    }
    if(strcmp(GRASP_PYTHON,"no")!=0){
        sprintf(tmp_str, " python module compiled (%s),", GRASP_PYTHON);
        strcat(str, tmp_str);
    }
    sprintf(tmp_str, "\n");
    strcat(str, tmp_str);
    sprintf(tmp_str, " Maximum segment size: nx=%d ; ny=%d ; nt=%d\n", _KIX, _KIY, _KITIME);
    strcat(str, tmp_str);
    tmp=grasp_compilation_information_input_drivers_versioned();
    sprintf(tmp_str, " Input drivers loaded: %s\n", tmp);
    strcat(str, tmp_str);
    free(tmp);
    tmp=grasp_compilation_information_input_transformers_versioned();
    sprintf(tmp_str, " Input transformers loaded: %s\n", tmp);
    strcat(str, tmp_str);
    free(tmp);
    tmp=grasp_compilation_information_output_segment_functions_versioned();
    sprintf(tmp_str, " Output segment functions loaded: %s\n", tmp);
    strcat(str, tmp_str);
    free(tmp);
    tmp=grasp_compilation_information_output_tile_functions_versioned();
    sprintf(tmp_str, " Output tile functions loaded: %s\n", tmp);
    strcat(str, tmp_str);
    free(tmp);
    tmp=grasp_compilation_information_output_current_functions_versioned();
    sprintf(tmp_str, " Output current functions loaded: %s\n", tmp);
    strcat(str, tmp_str);
    free(tmp);
    sprintf(tmp_str, " Path to resources: %s\n", RESOURCES_PREFIX);
    strcat(str, tmp_str);
    sprintf(tmp_str, " Sparse solver used: %s\n", GRASP_SPARSE_SOLVER);
    strcat(str, tmp_str);
    sprintf(tmp_str, " Build System: %s\n", GRASP_BUILD_SYSTEM);
    strcat(str, tmp_str);

    // Set GRASP_EXEC global variable with the absolute path to current executable
    sprintf(tmp_str, " Executable path: ");
    strcat(str, tmp_str);
    if(grasp_exec_file[0]=='/'){
        sprintf(tmp_str, "%s\n", grasp_exec_file);
        strcat(str, tmp_str);
    }else{
        sprintf(tmp_str, "undefined absolute path (%s)\n", grasp_exec_file);
        strcat(str, tmp_str);
    }
    return str;
}
...
time_t get_compilation_timestamp(){
    ...
    sprintf(str, "%s", GRASP_COMPILATION_DATE); // e.g. "2019-03-02 00:03:34 +100"
    str[19]='\0';
...
void grasp_compilation_information_print(FILE *f){
    char *compilation_information = grasp_compilation_information_str();
    fprintf(f, "%s", compilation_information);
    free(compilation_information);
}
...
char* _format_extension_with_version(
    char *space_delimited_extensions,
    char *space_delimited_versions
) {
    size_t extension_count = _count_spaces(space_delimited_extensions) + 1;
    size_t version_count = _count_spaces(space_delimited_versions) + 1;

    assert(extension_count == version_count);

    // We need strlen(space_delimited_extensions) + 1 (length of all extension names + following space)
    // + strlen(versions) + 1 * number of versions + \0
    size_t max_result_len = strlen(space_delimited_extensions)
        + strlen(space_delimited_versions)
        + version_count
        + 3;
    char *result = (char*)trackmem_malloc(sizeof(char)*max_result_len);
    assert(result!=NULL);
    ...
    char const * version;
    while((extension = strtok_r(space_delimited_extensions, " ", &space_delimited_extensions))) {
        version = strtok_r(space_delimited_versions, " ", &space_delimited_versions);
...
char *grasp_compilation_information_input_drivers_versioned(){
    ...
    return _format_extension_with_version(extensions, versions);
}

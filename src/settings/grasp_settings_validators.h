...
// This validator checks that product.fitting is true when the user wants to dump the simulated sdata file
...
// It checks that if the measurement type with cut off (from 51 to 55) is present, this is mandatory
...
// The parameter has the same number of elements that other element or 0 (don't defined). Argument 1=name of second element 
...
// The parameter has the same number of elements than the wavelengths in the settings file
...
// This validator is specific for the number of elements stimates and weigths, which has to be 0 or the same than the initial guess minus difference_order 
...
// To validate the option to add (or not) random noise to the measurements
...
// The parameter does not contain twice the same value different than 0

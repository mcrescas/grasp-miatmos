...
static bool retrieval_initiated=false;
...
grasp_output_stream *grasp_controller_get_trackmem_stream(){
    return &controller_trackmem_stream;
}
...
int grasp_controller_read_settings(int argc, char** argv, grasp_settings **settings, yamlsettings_parser_settings_file_mode settings_file_mode) {
    ...
    if (argc < 2) {
        ...
        return GRASP_ERROR_CONTROLLER_READ_SETTINGS_NOT_ENOUGH_ARGUMENTS;
    }

    // Read config file
    status_code=grasp_settings_read(&controller_stream, &dictionary, argc - 1, (const char **)&argv[1],settings_file_mode);
    // In case of error we have to initialize the controller_trackmem_stream to avoid errors
    // If there is not error we also initialize it but set to the proper output file
    if(status_code<0){
        grasp_output_stream_initialize("screan", &controller_trackmem_stream);
        return status_code;
    }else{
        grasp_output_stream_initialize(((grasp_settings *) dictionary->settings)->controller.track_mem_stream, &controller_trackmem_stream);
    }
...
int grasp_controller_initialize_inversion(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_processing_functions_t *functions, grasp_results_t *results) {
    ...
    // Init the functions structure first of all because in case of error,
    // the dealocator will work properly. Otherwise mem leak
    memset(functions, 0, sizeof(grasp_processing_functions_t)); 
    memset(results, 0, sizeof(grasp_results_t)); 
    
    
    status = grasp_input_initialize_tile_description(settings->input.nfiles,settings->input.files, tile_description);
...
int grasp_controller_invert_tile_sequential(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions) {
    ...
    status=grasp_init_inversion(&settings->retrieval);
    if (status<0){
        trackmem_free(segment);
        trackmem_free(output);
        return GRASP_ERROR_INPUT_INIT_INVERSION+status;
    }else{
    ...
    grasp_controller_iteration_callback_initialize(settings,segment,output,tile_description, functions);
    
    // Iterate over segments
    while((id_inversion=grasp_controller_get_next_inversion(&tile_description->dimensions, iinversion))>=0){
        status=grasp_controller_call_inversion(settings, segment, output, tile_description, results, functions, iinversion, id_inversion);
        if (status<0){
            trackmem_free(segment);
            trackmem_free(output);
            return status;
...
order_t *grasp_controller_order_callback(int worker_rank, const void *master_info, bool *no_more_order, int *num_orders) {
    order_t *order;
    char label[255 + 1];
    int id_inversion;
    ...
    *num_orders = ninversion - 1; // number of sent orders
...
int grasp_controller_collect_callback(const result_t *result, const void *master_info) {
    my_master_info_t *my_master_info;
    ...
    if(my_result_payload->status==GRASP_ERROR_CONTROLLER_RETRIEVAL_WANTS_TO_FINISH){
        return GRASP_ERROR_CONTROLLER_RETRIEVAL_WANTS_TO_FINISH;
    }
    if(my_result_payload->status==GRASP_ERROR_CONTROLLER_SEGMENT_NOT_INVERTED){
        nerror_segment++;
    ...
    return grasp_controller_post_process_segment((grasp_settings *) settings, (grasp_segment_t *) input_segment, (output_segment_general *) output_segment, 
					  (grasp_tile_description_t *) tile_description, (grasp_results_t *) results, (grasp_processing_functions_t *) functions 
                                          );
}

void grasp_controller_progress_info_callback(int num_orders_performed, const result_t *result, const void *master_info) {
    my_master_info_t *my_master_info;
    const grasp_tile_dimensions_t *tile_dimensions;
...
#if 0
    /* currently not used (but can serve as a base for additional progression info) */
    const my_result_payload_t *my_result_payload;
    int ninversion;
    my_result_payload = grasp_mpi_engine_get_result_payload(result);
    assert(my_result_payload != NULL);
    ninversion = my_result_payload->ninversion;
#endif
...
int grasp_controller_invert_tile_mpi(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions) {
    ...
    if (grasp_mpi_engine_is_worker()) {
        status = grasp_init_inversion(&settings->retrieval);
        if (status<0) return GRASP_ERROR_INPUT_INIT_INVERSION+status;
    }
...
int grasp_controller_invert_tile(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions) {
    ...
    return grasp_controller_invert_tile_mpi(settings, tile_description, results, functions);
#else
    ...
    return grasp_controller_invert_tile_sequential(settings, tile_description, results, functions);
#endif
}
...
int grasp_controller_processor_unit(const grasp_settings *settings, const grasp_segment_t *segment, output_segment_general *output, const grasp_tile_dimensions_t *tile_description, int iinversion) {
    ...
    // Init output
    memset(output,0,sizeof(output_segment_general));
        ...
        if (segment->sdata.npixels < 0) {
            ...
            if (segment->sdata.npixels > 0) {                    
                if (settings->controller.perform_retrieval == true) {
                    ...
                    grasp_controller_iteration_callback_setup(icol,irow,itime); // Prepare the global variables to receive callback functions from the inversion
                    inversion_result=grasp_input_inversion(&settings->retrieval, &segment->sdata, segment->iguess, &segment->edges, output);

                    benchmark_stop(&benchmark);
                    
                    output->retrieval.information.delta_ut=benchmark.delta_ut;
                    output->retrieval.information.delta_ct=benchmark.delta_ct;
                    
                    algorithm_ut+=benchmark.delta_ut;
                    algorithm_ct+=benchmark.delta_ct;
                    
                    // Dumping fitting SDATA file
                    grasp_output_stream_initialize(settings->output.simulated_file, &grasp_stream);
                    ...
                    if (grasp_output_stream_writable(&grasp_stream)) {
                        if(settings->retrieval.products.retrieval.fit){
                            grasp_input_dump_sdata(f, &(output->retrieval.fit.segment_fit));
                        }else{
                            gos_fprintf_flushed(&controller_stream, "WARNING: Fitting segment cannot be dumped because product.retrieval.fit is false\n");
                        }
                    }
                    ...
                    if(inversion_result==-1){
                        ...
                        result=GRASP_ERROR_CONTROLLER_SEGMENT_NOT_INVERTED;
                    }else if(inversion_result==-2){
                        fprintf(stderr,"FATAL ERROR: Retrieval code can not continue and it is stopped.\n");
                        result=GRASP_ERROR_CONTROLLER_RETRIEVAL_WANTS_TO_FINISH;
                    }else{
                ...
                f = grasp_output_stream_open(&grasp_stream, settings, segment, output, tile_description, icol, irow, itime);
                if (grasp_output_stream_writable(&grasp_stream)) {
                    grasp_input_dump_sdata(f, &(segment->sdata));
                }
                grasp_output_stream_close(&grasp_stream);
                grasp_output_stream_destroy(&grasp_stream);
...
int grasp_controller_post_process_segment(grasp_settings *settings, grasp_segment_t *segment, output_segment_general *output, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions) {
    ...
    if (grasp_controller_segment_is_invertible(segment->sdata.id_inversion, settings, &(tile_description->dimensions))==true && settings->controller.perform_retrieval==true){
        ...
        for (i = 0; i < settings->output.nsegment_output_function; i++) {
            ...
            status=functions->segment_output_functions[i].function(&stream, settings, segment, output, tile_description, icol, irow, itime);
            grasp_output_stream_destroy(&stream);
            if (status<0) return GRASP_ERROR_OUTPUT_SEGMENT_FUNCTION+status;
        }
        ...
        for (i = 0; i < settings->output.ncurrent_output_function; i++) {
            ...
            grasp_output_stream_destroy(&stream);
            if (status<0) return GRASP_ERROR_OUTPUT_CURRENT_FUNCTION+status;
        }
    }
...
int grasp_controller_call_inversion(grasp_settings *settings, grasp_segment_t *segment, output_segment_general *output, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions, int iinversion, int id_inversion) {
    ...
    if(status==GRASP_ERROR_CONTROLLER_RETRIEVAL_WANTS_TO_FINISH){
        return GRASP_ERROR_CONTROLLER_RETRIEVAL_WANTS_TO_FINISH;
    }
    if(status==GRASP_ERROR_CONTROLLER_SEGMENT_NOT_INVERTED){
        nerror_segment++;
        ...
        //return GRASP_ERROR_CONTROLLER_SEGMENT_NOT_INVERTED;
    }
    
    return grasp_controller_post_process_segment(settings, segment, output, tile_description, results, functions);
}
...
int grasp_controller_manage_tile(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_results_t *results, grasp_processing_functions_t *functions) {
    ...
    for (i = 0; i < settings->output.ntile_output_function; i++) {
        ...
        status=functions->tile_output_functions[i].function(&stream, settings, tile_description, results);
        grasp_output_stream_destroy(&stream);
        if (status<0) return GRASP_ERROR_OUTPUT_TILE_FUNCTION+status;
    }
...
int grasp_controller_free_results_description(grasp_results_description *result_description){
    int i;
    
    if (result_description!=NULL){
        if(result_description->results!=NULL){
            grasp_output_destroy_result(result_description->description, result_description->results);
            free(result_description->results);
        }

        result_description->results = NULL;

        if(result_description->description!=NULL){
            grasp_input_deallocate_expanded_files(result_description->description->input_files, result_description->description->ninput_files);
            if (result_description->description->used_files != NULL) {
                for (i = 0; i < result_description->description->nused_files; i++) {
                    free(result_description->description->used_files[i]);
                }
                free(result_description->description->used_files);
            }
            free(result_description->description);
        }
...
#ifdef USE_MPI // MPI mode: only workers finalize the inversion
    if (grasp_mpi_engine_is_worker()) {
      if (settings != NULL) grasp_finalize_inversion(&settings->retrieval);
    }
#else // sequential mode
    if (retrieval_initiated){
        grasp_finalize_inversion(&settings->retrieval);
        retrieval_initiated=false;
    }
#endif
    ...
    if (grasp_mpi_engine_is_master()) {
        ...
        if(functions!=NULL){
            ...
            if(functions->driver!=NULL){
                ...
                if(status<0) return GRASP_ERROR_DRIVER+status;
                
                trackmem_free(functions->driver);
            }

            if(functions->ntransformers>0){
                for (i = 0; i < functions->ntransformers; i++) {
                    status = functions->transformers[i].close();
                    if(status<0) return GRASP_ERROR_TRANSFORMER+status;
                }
                
                trackmem_free(functions->transformers);
            }
            
            if(functions->ntile_output_functions>0){
            ...
            if(functions->nsegment_output_functions>0){
            ...
            if(functions->niteration_output_functions>0){
                for (i = 0; i < functions->niteration_output_functions; i++) {
                    status = functions->iteration_output_functions[i].close();
                    if(status<0) return GRASP_ERROR_OUTPUT_ITERATION_FUNCTION+status;
                }

                trackmem_free(functions->iteration_output_functions);
            }
            
            if(functions->ncurrent_output_functions>0){
...
bool already_initialized(int n, char already_in_use[GRASP_MAX_OUTPUT_FUNC][_GBL_FILE_PATH_LEN], char *current_function){
    int i;
    
    for (i = 0; i < n; i++) {
        if (strcmp(already_in_use[i],current_function)==0){
            return true;
        }
    }
...
int grasp_controller_initialize_functions(grasp_settings *settings, grasp_tile_description_t *tile_description, grasp_processing_functions_t *functions) {
    ...
    // Get driver
    functions->driver = (grasp_input_driver_t *) trackmem_malloc(sizeof (grasp_input_driver_t) );
    *functions->driver = grasp_input_driver_get_function(settings->input.driver_name);
    ...
    // For iterations, if the function is already initialized we use it, otherwise initilize it now
    functions->niteration_output_functions = settings->output.niteration_output_function;
    if(functions->niteration_output_functions>0){
        functions->iteration_output_functions = (grasp_output_segment_function_t *) trackmem_malloc(sizeof (grasp_output_segment_function_t) * functions->niteration_output_functions);
        assert(functions->iteration_output_functions!=NULL);
        for (i = 0; i < functions->niteration_output_functions; i++) {
            functions->iteration_output_functions[i] = (grasp_output_segment_function_t) grasp_output_get_segment_output_funtion(settings->output.iteration_output_function[i]);
            if (! already_initialized(settings->output.nsegment_output_function, settings->output.segment_output_function, settings->output.iteration_output_function[i])){
                status = functions->iteration_output_functions[i].init(settings, tile_description);
            }
            if(status<0) return GRASP_ERROR_OUTPUT_ITERATION_FUNCTION+status;
        }
    }

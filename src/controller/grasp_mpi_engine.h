 ...
 * arguments to NULL.
 */
typedef order_t * (*order_callback_t)(int worker_rank, const void *master_info, bool *no_more_order, int *num_orders); /* master_info is optional and can be left to NULL 
															    * no_more_order must be set to true by the callback routine when
															    
															    * num_orders must be given the address of a variable in which the number of orders previously performed will be stored
...
extern order_t *grasp_mpi_engine_dummy_order_callback(int worker_rank, const void *master_info, bool *no_more_order, int *num_orders); /* called by the master to prepare an order for a worker */

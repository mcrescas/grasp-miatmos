...
! *** data_wl --  type contains all wavelength dependent data
      ...
      real(kind=C_FLOAT)          ::  tod(NBVM)  ! total optical depth; meas_type = 11
      real(kind=C_FLOAT)          ::  aod(NBVM)  ! aerosol optical depth; meas_type = 12
      real(kind=C_FLOAT)          ::  aaod(NBVM) ! absorption aerosol optical depth; meas_type = 13
      real(kind=C_FLOAT)          ::  htod(NBVM) ! hyperspectral total optical depth; meas_type = 14
      ...
      real(kind=C_FLOAT)          ::  p11_rel_ang(NBVM) ! p11/p11(given_angle) phase matrix element; meas_type = 27
      real(kind=C_FLOAT)          ::  p12_rel(NBVM)     ! -p12/p11 phase matrix element;             meas_type = 28
      real(kind=C_FLOAT)          ::  p11_intd(NBVM) ! p11 phase matrix element integrated in scattering angle range [ang1,ang2]; meas_type = 51 defined as [thetav, phi]
      real(kind=C_FLOAT)          ::  p11_intd_cut_off_1(NBVM) ! p11 phase matrix element integrated in scattering angle range [ang1,ang2] for rmax1 given in settings; meas_type = 52 defined as [thetav, phi]
      real(kind=C_FLOAT)          ::  p11_intd_cut_off_2(NBVM) ! p11 phase matrix element integrated in scattering angle range [ang1,ang2] for rmax2 given in settings; meas_type = 53 defined as [thetav, phi]
      real(kind=C_FLOAT)          ::  p11_intd_cut_off_3(NBVM) ! p11 phase matrix element integrated in scattering angle range [ang1,ang2] for rmax3 given in settings; meas_type = 54 defined as [thetav, phi]
      real(kind=C_FLOAT)          ::  p11_intd_cut_off_4(NBVM) ! p11 phase matrix element integrated in scattering angle range [ang1,ang2] for rmax4 given in settings; meas_type = 55 defined as [thetav, phi]

      real(kind=C_FLOAT)          ::  LS(KVERTM) ! lidar signal;                 meas_type = 31
      real(kind=C_FLOAT)          ::  RL(KVERTM) ! Raman lidar signal;           meas_type = 32
      real(kind=C_FLOAT)          ::  DPAR(KVERTM) ! polarization lidar par;     meas_type = 33
      real(kind=C_FLOAT)          ::  DPER(KVERTM) ! polarization lidar per;     meas_type = 34
      real(kind=C_FLOAT)          ::  DP(KVERTM) ! depolarization ratio;         meas_type = 35
      real(kind=C_FLOAT)          ::  VEXT(KVERTM) ! vertical extinction profile;meas_type = 36
      real(kind=C_FLOAT)          ::  VBS(KVERTM) ! vertical backscatter profile;meas_type = 39
      ...
      real(kind=C_FLOAT)          ::  I_rel_sum(NBVM) ! Relative Stokes parameter I/sum(I(1:NBVM)); meas_type = 45
      real(kind=C_FLOAT)          ::  P_rel(NBVM)     ! linear polarization sqrt(Q*Q+U*U)/I;        meas_type = 46
      ...
      real(kind=C_FLOAT)          ::  MU(KIP)    ! crosstalk between par/per polarized lidar channels
      integer(kind=C_INT)         ::  ind_wl_i   ! index of initial wl in wave length array for retrieved parameters (initial wl differs from recieved, e.g. RAMAN) 

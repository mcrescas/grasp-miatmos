...
! 11 - tod(wl)        - total optical depth
! 12 - aod(wl)        - aerosol optical depth
! 13 - aaod(wl)       - aerosol optical depth
...
! 27 - p11(angle,wl)/p11(given_angle,wl)  - relative scattering matrix element P11
! 28 - -p12(angle,wl)/p11(angle,wl)  - relative scattering matrix element P12

! 31  - LS(height,wl)      - lidar signal
! 32  - RL(height,wl)      - Raman Lidar signal
! 33  - LS_PAR(height,wl)  - polarized lidar signal (parallel to sounding beam)
! 34  - LS_PER(height,wl)  - polarized lidar signal (perpendicular to sounding beam)
...
! 44 - P(angle,wl)    - polarization sqrt(Q*Q+U*U)
! 45 - P(angle,wl)    - I(angle,wl)/sum(I(1:nmeas,wl)) - Relative Stokes parameter I
! 46 - P(angle,wl)    - P(angle,wl)/I(angle,wl) - degree of linear Polarization sqrt(Q*Q+U*U)/I

! 51 - p11_intd(angle,wl) - phase matrix element integrated in [ang1,90] and [90,ang2] ranges
! 52 - p11_intd_cut_off_1(angle,wl) - phase matrix element integrated in [ang1,90] and [90,ang2] ranges for rmax1
! 53 - p11_intd_cut_off_2(angle,wl) - phase matrix element integrated in [ang1,90] and [90,ang2] ranges for rmax2
! 54 - p11_intd_cut_off_3(angle,wl) - phase matrix element integrated in [ang1,90] and [90,ang2] ranges for rmax3
! 55 - p11_intd_cut_off_4(angle,wl) - phase matrix element integrated in [ang1,90] and [90,ang2] ranges for rmax4
...
! Search for index (iMN) of first measurement with first noise (MNOISEI(ip,iw)=1)
! If I and AOD meas are present I has to be 1st noise
          JJS_1 = 0
          iw_1  = 0
          ip_1  = 0
...
LOOP_meas_type : do IP=1,NIP
         ...
         if(iMN .lt. 1 .or. iMN .gt. RIN%NOISE%INOISE) then
            ...
            write(*,*) 'Please check noise definition in settings file.'
            write(*,*) 'This definition has to match with data to be processed.'
            stop 'stop 1 in covariance_matrix_pixel'         
         endif !
         if(                                   &
            (meas_type .ge. meas_type_Q   .and. meas_type .le. meas_type_U .and. RIN%iPOBS .le. 2) .or.  &
            (meas_type .eq. meas_type_P) .or. &
...
enddo LOOP_WL
      ...
      IF(INVSING .eq. 0 .or. (INVSING .gt. 0 .and. ipix .eq. 1)) THEN
        ...
        END SELECT ! KL
        if ( iw_1 .eq. 0 .or. IP_1 .eq. 0 .or. JJS_1 .eq. 0 ) then
          write(*,'(/,a)') 'An issue in covariance matrix with noise order for measurements:'
          write(*,'(4(i0,2x),3es12.4,a)') &
          iw_1,IP_1,JJS_1,iMN,XIK1,CS1,pixel_vec%FS(JJS_1), &
          '  -  iw_1,IP_1,JJS_1,iMN,XIK1,CS1,pixel_vec%FS(JJS_1)'
          write(*,'(a)') 'Мeasurement type with noise #1 must be present in pixel #1.'
          stop 'stop 2 in covariance_matrix_pixel'
        endif
      ENDIF ! INVSING .eq. 0 .or.
      ARES2 = CS1/XIK1 
...
! ----------------------------------------------------------------------
      ...
      if(lland .eqv. .true.) then
        do ipix=1,npixels
          if(NINT(segment%pixels(ipix)%land_percent) .eq. 0.0) then
      ...
      if(lwater .eqv. .true.) then
        do ipix=1,npixels
          if(NINT(segment%pixels(ipix)%land_percent) .eq. 100.0) then
...
!	sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
      subroutine set_aerosol_only_derivative_mask ( RIN, dermask_aerpar )
...
! ----------------------------------------------------------------------
      type(retr_input_settings), intent(in) :: RIN
      logical, dimension(KPARS), intent(out) :: dermask_aerpar
! ----------------------------------------------------------------------
      integer :: par_type
      integer :: IDIM1, ibeg, iend
! ----------------------------------------------------------------------
      dermask_aerpar(1:RIN%KNSING) = .true.
      if(RIN%KNSING .gt. RIN%KNSINGF) then
        dermask_aerpar(RIN%KNSINGF+1:RIN%KNSING) = .false.
      endif
      ...
      do IDIM1=1,RIN%NDIM%n1
        ...
        if(par_type .gt. par_type_surface_beg .and. par_type .lt. par_type_surface_end) then
          if(RIN%NDIM%par_retr(IDIM1)) then
            ibeg = RIN%NDIM%ISTARSING(1,IDIM1)
            iend = ibeg + sum(RIN%NDIM%n3(1:RIN%NDIM%n2(IDIM1),IDIM1)) - 1
            dermask_aerpar(ibeg:iend) = .false.
          endif
        endif
      enddo ! IDIM1

      return
      end subroutine set_aerosol_only_derivative_mask

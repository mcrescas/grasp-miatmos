...
! sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
      ...
      subroutine gradient_apriori_smooth_term ( KN, APS, SMSING, SMSEST, FFS )
!******************************************************
!*** Subroutine for calculating smoothness term of residual ***
!***
!
!   INPUT:
!      KN          I        - number of parameters
!      APS         R(KN)    - vector of parameters
!      SMSING      R(KN,KN) - single pixel matrix of smoothness
!      SMSEST               - structire of smoothness estimates
!
!***
!
!   OUTPUT:
!       FFS     R      - smoothness term in gradient of minimized residual
!*******************************************************
...
! IN :
      ...
      type(smoothness_estimates), intent(in) :: SMSEST
!------------------------------------------------------------
! OUT :
      real, dimension(KN), intent(out) :: FFS
!------------------------------------------------------------
      real, dimension(KN) :: FFS_EST
integer :: i
!------------------------------------------------------------
      FFS(1:KN) = 0.0

      if ( SMSEST%apply ) then
! FFS(1:KN) = SMSING(1:KN,1:KN)*APS(1:KN) - DIF_WGT^T(1:KN,1:KN)*EST(1:KN)
        ! possibility to apply smoothness estimates
        FFS(1:KN) = MATMUL(SMSING(1:KN,1:KN),APS(1:KN))
        FFS_EST(1:KN) = MATMUL(TRANSPOSE(SMSEST%DIF_WGT(1:KN,1:KN)),SMSEST%EST(1:KN))
        FFS(1:KN) = FFS(1:KN) - FFS_EST(1:KN)
      else
        ! w/o smoothness estimates
        FFS(1:KN) = MATMUL(SMSING(1:KN,1:KN),APS(1:KN))
      endif
!do i=1,KN
!write(*,'(a,i0,a)') 'i = ',i,'  SMSING: '
!write(*,'(10es12.4)') SMSING(1:KN,i)
!enddo
!write(*,'(a,i0,a)') 'APS: '
!write(*,'(10es12.4)') APS(1:KN)
!write(*,'(a,i0,a)') 'FFS: '
!write(*,'(10es12.4)') FFS(1:KN)
...
! sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
      subroutine residual_apriori_smooth_term(IPRI,iu,KN,APS,SMSING,SMSEST,AA)
!******************************************************
!*** Subroutine for calculating smoothness term of residual ***
!***
!
...
!      APS     R(KN)    - vector of parameters
!      SMSING  R(KN,KN) - single pixel matrix of smoothness
!      SMSEST           - structure of smoothnees estimates
!
!***
!
!   OUTPUT:
!       AA     R      - residual APS^T SMSING APS
!*******************************************************
      use mod_smoothness_estimates
...
! LOCAL ::
      integer :: i, i1
      real, dimension(KN) :: FFS
      real, dimension(KN) :: FFS_WGT
      real, dimension(KN,KN) :: SMSING_WGT
!------------------------------------------------------------
      AA = 0.0

      if ( SMSEST%apply ) then
goto 88
! AA = (DIF(1:KN,1:KN)*APS(1:KN)-EST(1:KN))^T * WGT(1:KN) * (DIF(1:KN,1:KN)*APS(1:KN)-EST(1:KN))
!
! APS^T(1:KN) * DIF^T(1:KN,1:KN)*DIF_WGT(1:KN,1:KN) * APS(1:KN,1:KN) =
! [ DIF^T(1:KN,1:KN)*DIF_WGT(1:KN,1:KN) = SMSING(1:KN,1:KN) ] =
! APS^T(1:KN) * SMSING(1:KN,1:KN) * APS(1:KN,1:KN)
!
! AA = APS^T(1:KN) * DIF^T(1:KN,1:KN)*DIF_WGT(1:KN,1:KN) * APS(1:KN)
!      - 2*APS^T(1:KN)*DIF^T(1:KN,1:KN)*WGT(1:KN)*EST(1:KN)
!      + EST^T(1:KN)*WGT(1:KN)*EST(1:KN)
!
        SMSING_WGT(1:KN,1:KN) = &
        MATMUL(TRANSPOSE(SMSEST%DIF(1:KN,1:KN)),SMSEST%DIF_WGT(1:KN,1:KN))
        FFS_WGT(1:KN) = MATMUL(SMSING_WGT(1:KN,1:KN),APS(1:KN))
        AA = DOT_PRODUCT(FFS_WGT(1:KN),APS(1:KN))
        FFS(1:KN) = SMSEST%WGT(1:KN)*SMSEST%EST(1:KN)
        FFS_WGT(1:KN) = MATMUL(TRANSPOSE(SMSEST%DIF(1:KN,1:KN)),FFS(1:KN))
        AA = AA - 2.0 * DOT_PRODUCT(FFS_WGT(1:KN),APS(1:KN))
        AA = AA + DOT_PRODUCT(SMSEST%EST(1:KN),FFS(1:KN))
88  continue
!
! AA = (DIF(1:KN,1:KN)*APS(1:KN)-EST(1:KN))^T * WGT(1:KN) * (DIF(1:KN,1:KN)*APS(1:KN)-EST(1:KN))
!
!goto 99
        ! possibility to apply smoothness estimates
        FFS(1:KN) = MATMUL(SMSEST%DIF(1:KN,1:KN),APS(1:KN))
        FFS(1:KN) = FFS(1:KN) - SMSEST%EST(1:KN)
        FFS_WGT(1:KN) = SMSEST%WGT(1:KN) * FFS(1:KN)
        AA = DOT_PRODUCT(FFS(1:KN),FFS_WGT(1:KN))
99 continue
      else
        ! test w/o smoothness estimates
        FFS(1:KN) = MATMUL(SMSING(1:KN,1:KN),APS(1:KN))
        AA = DOT_PRODUCT(FFS(1:KN),APS(1:KN))
      endif

      if(AA .lt. 0.) then
        if(IPRI) write(iu,*) AA,'-AA, negative, if(AA .lt. 0.) AA=0.;', &
                                      ' in residual_apriori_smooth_term !!!'
        AA = 0. ! OD
      endif ! AA

!do i=1,KN
!write(*,'(a,i0,a)') 'i = ',i,'  DIF in residual_apriori_smooth_term'
!write(*,'(10es12.4)') SMSEST%DIF(1:KN,i)
!enddo


!do i=1,KN
!write(*,'(a,i0,a)') 'i = ',i,'  SMSING in residual_apriori_smooth_term'
!write(*,'(10es15.7)') SMSING_WGT(1:KN,i)
!enddo
!write(*,'(a,es12.4,a)') 'AA =',AA,'  in residual_apriori_smooth_term'
!write(*,'(10es12.4)') APS(1:KN)
!write(*,'(a,i0,a)') 'FFS: '
!write(*,'(10es12.4)') FFS(1:KN)
!stop 999

module inversion_subsystem
    ...
    use mod_sdata,                                     only : get_pixel_wl, set_pixel_wl_i
    use mod_stop_report
...
include 'mpif.h'
  ...
  contains
    ...
    subroutine inversion_forward_model(                &
        ...
        integer,dimension(KW,KIMAGE)                           :: ind_wl_i
...
!        write (*,*) 'in inversion_forward_model'
!          do ipix=ipixstart,ipixstop
!             write(*,*) 'ipix, ind_wl', ipix, ind_wl(1:nwl(ipix),ipix)
!             write(*,*) 'ipix, ind_wl_i', ipix, ind_wl_i(1:nwl(ipix),ipix)
!           do i=1,NHVP_meas
!              write(*,*) 'ipix,i, HVP_meas(i,ipix)',ipix,i, HVP_meas(i,ipix)
!           enddo
!          enddo
        ...
        do ipix=ipixstart,ipixstop
            ...
            call forward_model_pixel(                               &
                    ...
                    IWb,                                            &
                    IWe,                                            &
                    0,                                              & !MH This is only for BB calculation
                    lresult,                                        &
                    tau_mol(:,ipix),                                &
                    NHVP_meas,                                      &
                    HVP_meas(:,ipix),                               &
    ...
    subroutine inversion_jacobian_matrix(                   &
        ...
        do ipix=ipixstart,ipixstop
            ...
            LOOP_WL: do IW=1,nwl
                ...
                IWb=IW
                IWe=IW
!write(*,'(2(a,i0,3x))') 'in inversion_jacobian_matrix IW = ',IW,'nFS = ',segment_vec_fit(ipix)%nFS(IW)
!write(*,'(a)') 'segment_vec_fit, FS:'
!write(*,'(10es12.4)') segment_vec_fit(ipix)%FS(J1+1:J1+segment_vec_fit(ipix)%nFS(IW))
                ...
                LOOP_parameters: do I=1,RIN%KNSINGF
                    ...
                    if(I .eq. -26) then
                        ...
                        write(*,'(a)') 'AP_temp:'
                        write(*,'(10es12.4)') AP_temp(1:RIN%KNSINGF)
                    endif

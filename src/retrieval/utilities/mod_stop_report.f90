...
! **********************************************************************
    subroutine print_stop_report()
        ...
        write(error_unit,'(a)') 'Current segment data can not be processed.'
...
! **********************************************************************
  subroutine g_error_aux(msg, filename, line)
    ...
    write(error_unit,'(/,"A new error occured at: ",A,":",I0,":",A,A)') &
      filename, line, NEW_LINE('A'), trim(msg)
    if (stop_report%status) then
      write(error_unit,'("Fatal error: it masks an old error at: ",A,":",I0,":",A,A)') &
	    trim(stop_report%file), stop_report%line, NEW_LINE('A'), &
      trim(stop_report%message)
	    stop
    end if

    stop_report%status = .true.
    stop_report%file = filename
    stop_report%line = line
    stop_report%message = msg
    return

  end subroutine g_error_aux

...
! Ocean:
! 1). iBRDF=0: isotripic case
! 2). iBRDF=1: anisotropic case
!
! Land:
! Subroutine BRM chooses the BRM model according to the flags:
! 1). iBRDF=0: RPV model in combination with BPDF model (see flag description for
!              iBPDF). Function "BRDF_RPV"
! 2). iBRDF=1: Ross-Li model  in combination with BPDF model (see flag description for
!              iBPDF). Subroutine "LiSp_Ross"
! 3). iBRDF=2: new physical model based on (Litvinov et all. JQSRT (2012), 113, 2023-2039. doi:10.1016/j.jqsrt.2012.06.027.)
!           Here just intensity is calculated. BPDF model is chosen with flag iBPDF.
!            Subroutine "Refl_LitvBRM_R11"
! 4). iBRDF=9,iBPDF=9: new physical model (Litvinov et al. JQSRT (2012), 113, 2023-2039. doi:10.1016/j.jqsrt.2012.06.027.)
!                   Subroutine  Refl_mat_4p(n_el,n_par,tet0,tetv,phi0,phiv,x_vect,m_ind,Rm) for reflection matrix

! Surface polarization
! 1). iBPDF=-1: no surface polarization
! 2). iBPDF=0:  Maignan et al. BPDF model
! 3). iBPDF=1: BPDF model developted at SRON (Litinov et all.  Remote Sensing of Environment (2011), 115, 781–792;
!                                         Litinov et all.In book: Polarimetric Detection, Characterization, and Remote Sensing. Springer, 2011.)
! 4). iBPDF=9: new physical model (Litvinov et all.) always with iBRDF=9. Subroutine  Refl_mat_4p
...
!------------------------------------------------------
      Real(8), parameter :: pi_dp=3.141592653589793d0 !pi_dp=3.14159265358979323846264338327950
      Real(8), parameter :: deg_dp=180.d0/pi_dp
      Real(8), parameter :: rad_dp=pi_dp/180.d0
!------------------------------------------------------
...
!PL      LOGICAL, PARAMETER :: IWAVE = .TRUE. !.TRUE. !.FALSE.
!------------------------------------------------------
...
!XH               LOG(CHI) = (LOG(CHIS(IWV))-LOG(CHIS(IWV-1)))/(LOG(WVS(IWV))-LOG(WVS(IWV-1)))*(LOG(WV)-LOG(WVS(IWV)))+LOG(CHIS(IWV))
      ...
      END IF
!
      ALPHA = SQRT(13.D0*D*4.D0*pi_dp*(CHI+M)/WV)
!XH   scattering angle THETA in degree
      THETA = deg_dp*DACOS(-MUS*MUV-SQRT((1.D0-MUS*MUS)*(1.D0-MUV*MUV))*DCOS(PHI*rad_dp))
...
REAL(8) PURE FUNCTION FOAM_ALBEDO_first(WV)
...
!
	  ...
	  FOAM_ALBEDO_first=FOAM_ALBEDO
!
      RETURN
END FUNCTION FOAM_ALBEDO_first


!PL Modified according to Robet Frouin suggestions	
...
!
      ...
      REAL(8), PARAMETER, DIMENSION(NWV) :: WVS = (/0.6D0,0.85D0,1.02D0,1.65D0,2.25D0/) 
	  									!WVS = (/0.44D0,0.85D0,1.02D0,1.65D0,2.25D0/)
      REAL(8), PARAMETER, DIMENSION(NWV) :: ALB = (/1.00D0,0.60D0,0.50D0,0.15D0,0.00D0/)
!
      INTEGER :: I
!
      IF (WV .LE. WVS(1)) THEN
          FOAM_ALBEDO = ALB(1)
      ELSE 
		  FOAM_ALBEDO=exp(-1.75*((WV - 0.6)**0.99))
      END IF
...
Subroutine BRM_ocean(n_el,iBRDF,n_par,mu0,muv,phi,WAVE,x_vect,Rip)
!     iBRDF,iBPDF: flags for model
...
!     Rip          : reflection matrix
      ...
      complex(8) :: m
      real(8)    :: afoam
	  real(8), parameter:: FOAM_ALBEDO_440=0.22
	  LOGICAL  IWAVE 
...
!XH       Assume foam reflectance at 440nm is 22%
          afoam = FOAM_ALBEDO(WAVE)*FOAM_ALBEDO_440 !0.22D0
...
!        anisotropic case
      ...
      case(2)
!        simplest isotropic case
         call Refl_mat_ocean_2par(n_el,.true.,n_par,mu0,muv,phi0,phiv,x_vect(1:n_par),m,afoam,Rip)		 
	  case(9)
         call Refl_mat_3p_ocean(n_el,n_par,dacos(mu0),dacos(muv),  &
	                          phi0,phiv,x_vect(1:n_par),m,Rip)						  
      case default
         write(*,*) 'iBRDF=',iBRDF,' - unknown value (iBRM_water)'
...
!PL     iBRDF,iBPDF: flags for model
...
Subroutine BRM(n_el,iBRDF,iBPDF,n_par,n_par_i,mu0,muv,phi,WAVE,x_vect,Rip)
      Implicit none
      Integer,  intent (in) :: n_el,iBRDF,iBPDF
...
!XH    refractive index of land surface used in BPDF (may be changed below in case of snow surface)
       m = cmplx(1.5d0,0.d0)
!      Write(*,*) 'in  ', iBRDF,iBPDF,n_par,mu0,muv,phi,m
!      Write(*,*) x_vect
!      stop
...
!		 call Refl_mat_4p_simp(n_el,n_par,tet0,tetv,phi0,phiv,x_vect(1:n_par),m,Rip)
      case(11)
!        Kernel Model 
         call Kernel_BRF_Model(WAVE,n_par_i,mu0,0.d0,muv,phi,x_vect(1:n_par_i),R11)
      case default
         write(*,*) 'iBRDF=',iBRDF,' - unknown value'
...
!        BRDF + My BPDF model developed in SRON
         ...
         x_vect_p(4) = 1.5d0
         phiv=phi*rad_dp
         call R_Fren_PL_SRON(n_el,n_par_p,mu0,pi_dp,muv,phiv,x_vect_p,Rip)
!         Rip(1,1)= R11
         Rip(1,1)= Rip(1,1)+R11
      case(-1)
!        BRDF only
...
!
      ...
      xx=dsqrt(1.d0-cs*cs)
      yy=dsqrt(1.d0-cv*cv)
      ww=cs*cv+xx*yy*dcos(phi*rad_dp)
      aa=1.d0+gt*gt+2.d0*gt*ww
      FF2=(1.d0-gt*gt)/(aa**1.5d0)

      vv=xx/cs
      ww=yy/cv
      G=dsqrt(vv*vv+ww*ww-2.d0*vv*ww*dcos(phi*rad_dp))
!      FF3=1+(1-rho)/(1+G)
...
!XH   for potential computational error in cos_scat
      ...
      alpha=(pi_dp-tet_sc)*deg_dp
      Hot_Sp=(1.d0+1.d0/(1.d0+alpha/alpha0))

      Ross_K=Hot_Sp*((pi_dp*0.5d0-tet_sc)*cos_scat+sin_scat)/(mu+mu0)-pi_dp*0.25d0
...
!     here positive angle corresponds to backscattering direction
      ...
      t=dacos(cos_t)
      O=1.d0/pi_dp*(t-sin_t*cos_t)*sec_i_v
...
!PL     Kernel based BRF
Subroutine Kernel_BRF_Model(wave,n_par_i,mu0,phi0,mu,Phir,x_vect_i,Kernel_BRF)
	  Implicit none

      Integer, intent(in):: n_par_i
      Real(8), intent(in):: wave,mu0,phi0,mu,Phir,x_vect_i(1:n_par_i)
      Real(8),intent(out):: Kernel_BRF

	  Real(8) x_vect(1:n_par_i,1:3)
	  Real(8) R11,sum_norm
      
      Integer :: i

	  call BRF_param_dataset(wave,n_par_i,x_vect)
	  ...
	  Do i=1,n_par_i
	  	...
	  	call LiSp_Ross(n_par_i,mu0,0.d0,mu,Phir,x_vect(i,1:3),R11)
		
		Kernel_BRF=Kernel_BRF+x_vect_i(i)*R11

		sum_norm=sum_norm+x_vect_i(i)

	  end do	

	  Kernel_BRF=Kernel_BRF/sum_norm
	 ! x_vect_i(:)=x_vect_i(:)/sum_norm

End Subroutine Kernel_BRF_Model



Subroutine BRF_param_dataset (wave,n_par_i,x_vect)
  Implicit none

  Integer, intent (in):: n_par_i
  Real(8), intent(in):: wave
  Real(8), intent(out):: x_vect(1:n_par_i,1:3)

  INTEGER, PARAMETER :: n_w = 6, n_par_max=4
  REAL(8), PARAMETER, DIMENSION(n_w) :: waves_arr = (/0.443,0.490,0.56,0.670,0.865,1.02/)
  
  REAL(8), DIMENSION(n_par_max,n_w) :: k_iso	

  REAL(8), PARAMETER, DIMENSION(n_par_max) :: 	k_vol= 	(/0.3,1.0,0.200,0.0001/)
  REAL(8), PARAMETER, DIMENSION(n_par_max) ::	k_geom= (/0.3,0.1,0.001,0.0001/)
										
  real(8), parameter:: 	eps_wave=0.01

  integer i_w, i_kernal

  k_iso(1,1:n_par_i)= [0.002,0.004,0.007,0.120,0.200,0.250] !bare soil
  k_iso(2,1:n_par_i)= [0.048,0.050,0.100,0.040,0.520,0.510] !forest
  k_iso(3,1:n_par_i)= [0.070,0.120,0.300,0.500,0.600,0.700] !desert
  k_iso(4,1:n_par_i)= [0.010,0.055,0.100,0.080,0.300,0.290] !soil

!

  do i_w=1, n_w

	if (abs(waves_arr(i_w) - wave) .le. eps_wave) then
   
  		do i_kernal=1,n_par_i
  			x_vect(i_kernal,1)=k_iso(i_kernal,i_w)
			x_vect(i_kernal,2)=k_vol(i_kernal)
			x_vect(i_kernal,3)=k_geom(i_kernal)
		enddo
...
!
      ...
      Subroutine BRM_Fexp_OSH_XH(ILUT,n_el,land_percent,M_max,n_tet,mu,wt,WAVE, &
                                 iBRDF,iBPDF,n_par,n_par_i,x_vect,              &
                                 iBRM,n_par_water,x_vect_water,                 &
                                 R11,R21,R22,R31,R32,R33,salb)
!XH   output independent elements only
!XH   surface properties may be wavelength dependent
      Implicit none
      LOGICAL,    intent(in) :: ILUT
      integer,    intent(in) :: n_el
      Integer,    intent(in) :: n_par,n_par_i,M_max, n_tet, iBRDF,iBPDF
      Real,       intent(in) :: land_percent
      Real(8),    intent(in) :: WAVE
      Real(8),    intent(in) :: mu(1:n_tet),wt(1:n_tet),x_vect(1:n_par)
      Integer,    intent(in) :: n_par_water,iBRM
      Real(8),    intent(in) :: x_vect_water(1:n_par_water)
      Real,       intent(out):: salb
      Real, dimension(1:n_tet,1:n_tet,0:M_max), intent(out) :: R11,R21,R22
      Real, dimension(1:n_tet,1:n_tet,0:M_max), intent(out) :: R31,R32,R33
...
!     Here we use the fact that variable IGQ_BRM_FEXP is saved in the memory
!     and we need to calculate Gaussian quadratures just one time.
      If (RT_SOS_RES%IGQ_BRM_FEXP) then
         Do i3=1, n_phi
            Gw_p(i3)=Gw_p_old(i3)
            phi(i3) = phi_old(i3)
         end do
      ...
      IF (ILUT) THEN
!XH      only calculate surface albedo for LUT
         Do i1=1, n_tet
            Do i2=1, i1
               Do i3=1, n_phi
                  select case(nint(land_percent))
                  case(0)
                     call BRM_ocean(n_el,iBRM,n_par_water,mu(i1),mu(i2),(180.d0-phi(i3)*deg_dp), &
                                    WAVE,x_vect_water(1:n_par_water),Rm)
                  case(100)
                     call BRM(n_el,iBRDF,iBPDF,n_par,n_par_i,mu(i1),mu(i2),(180.d0-phi(i3)*deg_dp),&
                  ...
                  case default
                     ...
                     call BRM(n_el,iBRDF,iBPDF,n_par,n_par_i,mu(i1),mu(i2),(180.d0-phi(i3)*deg_dp),&
                                    WAVE,x_vect(1:n_par),Rm)
                     Rm=(1.-0.01*land_percent)*Rm_ocean+0.01*land_percent*Rm
                  end select

                  mu_mphi =Gw_p(i3)/pi_dp
!XH               calculated lower triangular matrix only for off-diagonal elements
                  R11(i2,i1,0)=R11(i2,i1,0)+ mu_mphi*Rm(1,1)
               End do !i3
            End do !i2
         End do !i1

!XH      using reciprocity relation for diagonal elements of submatrices
         do i1 = 1, n_tet
            do i2 = i1+1, n_tet
               R11(i2,i1,0)=R11(i1,i2,0)
            end do
         end do

!XH      calculate bi-hemispherical reflectance of surface
         salb = 0.
         do i1 = 1, n_tet
            salb = salb + sum(R11(i1,1:n_tet,0)*mu(1:n_tet)*wt(1:n_tet))*mu(i1)*wt(i1)
         end do
         if (salb .gt. 1.) salb=1.
         salb = 4.*salb
         return
...
!PL               necessary to take '180.0-phi(i3)'
               ...
               Do i_m=0,M_max
                  mu_mphi =Gw_p(i3)*dcos(i_m*phi(i3))/pi_dp
                  sin_mphi=Gw_p(i3)*dsin(i_m*phi(i3))/pi_dp
                  if (i1 .lt. i2) then
!XH                  calculated whole matrix only for off-diagonal elements
                     if (n_el .ge. 3) then
                        R21(i2,i1,i_m)=R21(i2,i1,i_m)+ mu_mphi*Rm(2,1)
                        R31(i2,i1,i_m)=R31(i2,i1,i_m)+sin_mphi*Rm(3,1)
                        R32(i2,i1,i_m)=R32(i2,i1,i_m)+sin_mphi*Rm(3,2)
                     end if
                  else
!XH                  calculated lower triangular matrix only for diagonal elements
                     R11(i2,i1,i_m)=R11(i2,i1,i_m)+ mu_mphi*Rm(1,1)
                     if (n_el .ge. 3) then
                        R22(i2,i1,i_m)=R22(i2,i1,i_m)+ mu_mphi*Rm(2,2)
                        R33(i2,i1,i_m)=R33(i2,i1,i_m)+ mu_mphi*Rm(3,3)
                        R21(i2,i1,i_m)=R21(i2,i1,i_m)+ mu_mphi*Rm(2,1)
                        R31(i2,i1,i_m)=R31(i2,i1,i_m)+sin_mphi*Rm(3,1)
                        R32(i2,i1,i_m)=R32(i2,i1,i_m)+sin_mphi*Rm(3,2)
                     endif
...
!XH   calculate bi-hemispherical reflectance of surface
      ...
      do i1 = 1, n_tet
         do i2 = 1, n_tet
            R11(i1,i2,:)=R11(i1,i2,:)*mu(i2)
            if (n_el .ge. 3) then
               R22(i1,i2,:)=R22(i1,i2,:)*mu(i2)
               R33(i1,i2,:)=R33(i1,i2,:)*mu(i2)
               R21(i1,i2,:)=R21(i1,i2,:)*mu(i2)
               R31(i1,i2,:)=R31(i1,i2,:)*mu(i2)
               R32(i1,i2,:)=R32(i1,i2,:)*mu(i2)
            end if
         end do
         salb = salb + sum(R11(i1,1:n_tet,0)*wt(1:n_tet))*mu(i1)*wt(i1)
      end do
      salb = 4.*salb
!
      End Subroutine BRM_Fexp_OSH_XH

!PL  Coherent Backscattering for infinite medium
 Real(8) Function CB_peak_mu(mu0,muv,tet,alb_s)
 Implicit none
 Real(8),parameter:: eps=0.00001,kl_ext=30.d0!,Coh_peak_ampl=0.5
 Real(8), intent(in):: tet,mu0,muv,alb_s
 Real(8) q, CB_peak,Coh_peak_ampl !,pi

  Coh_peak_ampl=0.7
!  Coh_peak_ampl=Coh_peak_ampl*alb_s+0.5
  q=2.*kl_ext*dcos(tet*0.5d0)

  If (dabs(tet-pi_dp) .gt. eps)  then
   CB_peak=Coh_peak_ampl*datan(q)/q
  else 
   CB_peak=Coh_peak_ampl
  endif

!  CB_peak_mu=1.+CB_peak*(mu0+muv)/2.
   CB_peak_mu=1.+CB_peak*mu0!*mu0
!  CB_peak_mu=1.+CB_peak*mu0*muv
...
!PL  Coherent Backscattering for infinite medium
 ...
 Implicit none
 Real(8),parameter:: eps=0.00001,kl_ext=30.d0,Coh_peak=0.3 !PL Test2	0.5
...
!
	  ...
	  Real(8) :: delta_foam, wind_speed

      alb    = x_vect(1)
      Fren_fr= x_vect(2)
      sig1   = x_vect(3)
      If (.not. iso) then
!         phiw    = x_vect(4)*rad_dp
...
!         sigmau2 = sig1
      ...
      If (iso) then
         f_pdf1=dexp(-(1.d0-mu_n1*mu_n1)/(2.d0*sig1*mu_n1*mu_n1))/(2.d0*pi_dp*sig1*mu_n1**3)
         ...
         f_pdf1= f_pdf1/(2.d0*pi_dp*sqrt(sigmac2*sigmau2)*mu_n1**3)
      end if

      call R_Fren_My(n_el,mu0,muv,del_phi*deg_dp,m,Rm)
...
!      call R_Fren(mu01,phii,muv1,phiv1,Rm)
      ...
      Rm=Fren_fr*Shad*f_pdf1*pi_dp/(4.d0*mu0*muv*mu_n1)*Rm

!      Rm=Fren_fr*f_pdf1*pi_dp/(4.d0*mu0*muv*mu_n1)*Rm

      Rm(1,1)=Fren_fr*alb+Rm(1,1)+(1.d0-Fren_fr)*A_foam


!PL calculation of delta_foam from wind speed
      
  !PL    wind_speed=max(0.,(2.*sig1-0.003)/0.00512) 
  !PL    delta_foam=2.95e-6 * (wind_speed**3.52)

  !PL	  Rm(1,1)=Fren_fr*alb + Rm(1,1) + delta_foam*A_foam
...
!PL reflection matrix for ocean based on
!PL isotropic and anisotropic Cox-Munk model
Subroutine Refl_mat_ocean_2par(n_el,iso,n_par,mu0,muv,phi0,phiv,x_vect,m,A_foam,Rm)
      Implicit none
      Logical,    intent(in) :: iso
      Integer,    intent(in) :: n_el,n_par
      Real(8),    intent(in) :: mu0,phi0,muv,phiv,x_vect(1:n_par)
...
!
      return
End Subroutine Refl_mat_ocean_2par
...
!     Here we use the fact that variable IGQ_BRM_HSPH is saved in the memory
!     and we need to calculate Gaussian quadratures just one time.
      If (.NOT. RT_SOS_RES%IGQ_BRM_HSPH) then
      ...
      If (boolean) then
         ...
         call R_Fren_My(n_el,mu0,muv,phiv/rad_dp,m_ind,Rm)
         call Shad_func(mu0,muv,k_shad*sig1,Shad)
        
         mu_n0 = (mu0+muv)/(2.d0*dsin(0.5d0*tet))
         f_pdf0=dexp(-(1.d0-mu_n0*mu_n0)/(2.d0*sig1*mu_n0*mu_n0))/(2.d0*pi_dp*sig1*mu_n0**3)

         Rm=Fren_fr*Shad*f_pdf0*Rm*pi_dp/(4.d0*mu0*muv*mu_n0)		 
          ...
          If (dabs(x_vect_old(2)-x_vect(2)) .gt. eps) then
            Do i2=1,n_bet
               ...
               f_pdf1(i2)=dexp(-(1.-mu_n1*mu_n1)/(2.d0*sig1)/mu_n1/mu_n1)/ &
	                               pi_dp/(2.*sig1)/mu_n1/mu_n1/mu_n1/mu_n1
            end do
          end if
...
Subroutine  Refl_mat_4p_simp(n_el,n_par,tet0,tetv,phi0,phiv,x_vect,m_ind,Rm)
      Implicit none
      Integer, parameter   :: n_bet=15,n_alf=9 !n_bet=20,n_alf=40!n_bet=8,n_alf=12 !n_bet=9,n_alf=25
      Integer, intent (in) :: n_el,n_par
      Real(8), intent (in) :: tetv,tet0,phi0,phiv,x_vect(1:n_par_max)
	  Complex(8),intent(in) :: m_ind
      Real(8), intent (out):: Rm(1:n_el,1:n_el)
...
!      Real(8),parameter:: sin_eps=0.00000001!, k_shad=1.6
      ...
      real(8), parameter:: eps=0.00000000001d0, sin_eps=0.00000001d0, sig_min=0.01d0
      Real(8), parameter:: par_free=4.d0/3.d0,  alb_m=0.2d0!alb_m
      Real(8), parameter:: Depol_factor=50. !0.005d0, sig2=0.005d0  !0.9!x_vect(5) ! for ocean
!      Real(8), parameter:: Depol=0. !0.005d0 !, sig2=0.2 !0.9d0  !0.9!x_vect(5)   ! for land
...
!XH   take advantage of new feature in Fortran95
      ...
      If (boolean) then
         ...
         case(3)
			...
			alb_n=0.
         case(4)
            sig2	=x_vect(4) !k_shad=1. !x_vect(4)
			alb_n=0.
         case(5)
			sig2	=x_vect(4) !k_shad=1. !x_vect(4) 
            alb_n	=x_vect(5)			
...
!		 sig2=x_vect(4)
         ...
         call R_Fren_My(n_el,mu0,muv,phiv/rad_dp,m_ind,Rm)
         call Shad_func(mu0,muv,sig2,Shad)

!PL Test1 		 
 !        R11_elem=1. !(1.-alb_m*dsin(tet/par_free)*dsin(tet/par_free))

		 R11_elem=(1.+alb_n*((pi_dp*0.5d0-tet)*cos_tet+dsqrt(1.-cos_tet*cos_tet)))/(1.+alb_n*pi_dp*0.5d0)


         mu_n0 = (mu0+muv)/(2.d0*dsin(0.5d0*tet))
         f_pdf0=dexp(-(1.d0-mu_n0*mu_n0)/(2.d0*sig2*mu_n0*mu_n0))/(2.d0*pi_dp*sig2*mu_n0**3)
...
!            Int_alfbet1_Fr=Int_alfbet1_Fr+Int_bet1_Fr*Gw_t(i2)
		  ...
		  Shad=dexp(-dtan((pi_dp-tet)*0.5d0))
   		  Rm=Rm*Int_alfbet1!/mu0/muv*Shad
...
!          Rm(1,1)=alb_s*R11_elem*CB_peak_new(tet)*Int_alfbet1/mu0/muv*Shad+Rm(1,1)
		  Rm(1,1)=alb_s*R11_elem*CB_peak_mu(mu0,muv,tet,alb_s)*Int_alfbet1/mu0/muv*Shad+Rm(1,1)
          if (n_el .ge. 3) then 
			...
			Rm(2,2)= Depol_22*alb_s*R11_elem*CB_peak_mu(mu0,muv,tet,alb_s)*Int_alfbet1/mu0/muv*Shad*cos_2etv*cos_2eti+Rm(2,2)
		  ...
		  call Shad_func(mu0,muv,k_shad*sig1,Shad)
          Rm(1,1)=alb_s*R11_elem*CB_peak_mu(mu0,muv,tet,alb_s)*Shad+Rm(1,1)
          if (n_el .ge. 3) then 
			...
			Rm(2,2)= Depol_22*alb_s*R11_elem*CB_peak_mu(mu0,muv,tet,alb_s)*Shad*cos_2etv*cos_2eti+Rm(2,2)
...
Subroutine  Refl_mat_4p_old_good(n_el,n_par,tet0,tetv,phi0,phiv,x_vect,m_ind,Rm)
      Implicit none
      Integer, parameter   :: n_bet=15,n_alf=9 !n_bet=11,n_alf=25 !n_bet=15,n_alf=31  !n_bet=9,n_alf=22 !n_bet=20,n_alf=40!n_bet=8,n_alf=12 !n_bet=9,n_alf=25
      Integer, intent (in) :: n_el,n_par
      Real(8), intent (in) :: tetv,tet0,phi0,phiv,x_vect(1:n_par_max)
	  Complex(8),intent(in) :: m_ind
      Real(8), intent (out):: Rm(1:n_el,1:n_el)
...
!      Real(8),parameter:: sin_eps=0.00000001!, k_shad=1.6
      ...
      real(8), parameter:: eps=0.00000000001d0, sin_eps=0.00000001d0, sig_min=0.005d0
      Real(8), parameter:: par_free=4.d0/3.d0,  alb_m=0.2d0!alb_m
!      Real(8), parameter:: Depol=0.005d0, sig2=0.005d0  !0.9!x_vect(5) ! for ocean
      Real(8), parameter:: Depol_factor=0.050 !5d0 !, sig2=0.2 !0.9d0  !0.9!x_vect(5)   ! for land

!PL Test 3
	  real(8) sig2,sig0, Depol
!      Real(8) alb_m
...
!XH   take advantage of new feature in Fortran95
  ...
  !          k_shad=x_vect(4)
			sig2=x_vect(4)
	!	    Depol=min(0.005,alb_s*Depol_factor)
		    Depol=alb_s*Depol_factor*dexp(-alb_s)			

         case(5)
  !          k_shad=x_vect(4)
			sig2=x_vect(4)
		    Depol=alb_s*x_vect(5)*dexp(-alb_s) !x_vect(5)min(0.005,alb_s*x_vect(5))
...
!          Rm(1,1)=alb_s*R11_elem*CB_peak_new(tet)*Int_alfbet1/mu0/muv*Shad+Rm(1,1)
		  ...
		  sig0=max(sig1,sig2)
		  call Shad_func(mu0,muv,k_shad*sig0,Shad)
        
          mu_n0 = (mu0+muv)/(2.d0*dsin(0.5d0*tet))

		  f_pdf0=dexp(-(1.d0-mu_n0*mu_n0)/(2.d0*sig0*mu_n0*mu_n0))/(2.d0*pi_dp*sig0*mu_n0**3)

          Rm=Fren_fr*Shad*f_pdf0*Rm*pi_dp/(4.d0*mu0*muv*mu_n0)		 
 
          Rm(1,1)=alb_s*R11_elem*CB_peak_mu(mu0,muv,tet,alb_s)+Rm(1,1)
          if (n_el .ge. 3) then 
...
!
      return
End Subroutine Refl_mat_4p_old_good
...
!      Real(8),parameter:: sin_eps=0.00000001!, k_shad=1.6
      ...
      real(8), parameter:: eps=0.00000000001d0, sin_eps=0.00000001d0, sig_min=0.005d0
      Real(8), parameter:: par_free=2.d0 !4.d0/3.d0!,  alb_m=0.2d0!alb_m
!      Real(8), parameter:: Depol=0.005d0, sig2=0.005d0  !0.9!x_vect(5) ! for ocean
      Real(8), parameter:: Depol_factor=0.040 !5d0 !, sig2=0.2 !0.9d0  !0.9!x_vect(5)   ! for land
...
!			Depol=0.005
  ...
  !          k_shad=x_vect(4)
			sig2=x_vect(4)
		    Depol=alb_s*x_vect(5)*dexp(-alb_s) !x_vect(5) !0.001 ! !x_vect(5)min(0.005,alb_s*x_vect(5))
...
!PL Test0 		 
!         R11_elem=1./(mu0+muv) !(1.-alb_m*dsin(tet/par_free)*dsin(tet/par_free))
!PL Test1 		 
		 R11_elem=(1.+alb_m*((pi_dp*0.5d0-tet)*cos_tet+dsqrt(1.-cos_tet*cos_tet)))/(1.+alb_m*pi_dp*0.5d0)

!PL Test 3
!         R11_elem=(1.+alb_m*((pi_dp*0.5d0-tet)*cos_tet+dsqrt(1.-cos_tet*cos_tet))/(muv+mu0))/(1.+alb_m*pi_dp*0.5d0)!/(muv+mu0))

!		 R11_elem=(1.-alb_m*dsin(tet/par_free)*dsin(tet/par_free))/(1.-alb_m) !/(mu0+muv))
...
!------------------------------------------------------
!Real(8), parameter :: pi_dp=3.141592653589793 !23846264338327950
!!Real(8), parameter :: dpi2=pi_dp*pi_dp
!Real(8), parameter :: deg_dp=180.d0/pi_dp
!Real(8), parameter :: rad_dp=pi_dp/180.d0
!------------------------------------------------------
...
!   Write(*,'(2f10.5,3f20.16)') dacos(dmui1)*deg_dp,phii*deg_dp,VI1o,VI2o,VI3o
...
!!!!!!!!!!!!!!!!!
   ...
   if (dabs(dabs(sin_phi_r)-dsqrt(1.-cos_phi_r**2)) .gt. 0.0001) then
    Write(*,*) 'Error sin_phi_r',dacos(dmur1)*deg_dp, sin_phi_r,dsqrt(1-cos_phi_r**2)
...
!      sin_eps=dsin(st_min*rad_dp)

      sin_tv=dsqrt(1.d0-muv*muv)
      sin_ti=dsqrt(1.d0-mu0*mu0)
      zz=muv*mu0+sin_ti*sin_tv*dcos(phi*rad_dp)
      cos_tet=-zz
      sin_tet=dsqrt(1.d0-cos_tet*cos_tet)

      If (dabs(sin_tv) .lt. sin_eps) then
         tet0v=dacos(muv)
         tet0v=dabs(tet0v-st_min*rad_dp)
         muv=dcos(tet0v)
         sin_tv=dsqrt(1.d0-muv*muv)
         zz=muv*mu0+sin_ti*sin_tv*dcos(phi*rad_dp)
         cos_tet=-zz
...
!------------------------------------------------------
      ...
      DO i=1,m_gauss
        var1= dcos(pi_dp*(dble(i)-0.25D0)/(dble(ngauss)+0.5D0))
	    var3=1.d0
...
!PL Subroutine for calculation of White Sky Albedo (isotropic BiHemispherical Reflectance) 
 subroutine BHRiso(iBRDF, iBPDF, iBRM_water, land_percent, &
                  n_par_land, n_par_land_i, x_vect_land, &
                  n_par_water, x_vect_water,WAVE, &
                  BHR_iso )
 implicit none

 integer,   intent(in)    ::  iBRDF, iBPDF, iBRM_water
 real(8),   intent(in)    ::  land_percent, WAVE
 integer,   intent(in)    ::  n_par_land, n_par_land_i, n_par_water
 real(8),   intent(in)    ::  x_vect_land(1:n_par_land), x_vect_water(1:n_par_water)

 real, 		 intent(out)   ::  BHR_iso
!	------------------------------------------------------------------------------------
!   LOCAL:

 integer, 	parameter 	::	n_el=1, n_tet_max=40, n_phi_max=40, land_percent_thresh=20
 real(8), 		parameter	:: 	low_lp=0.d0, up_lp=pi_dp,  low_lt=0., up_lt=1.
 
 real(8) 	mu(1:n_tet_max), phi(1:n_phi_max), Gw_t(1:n_tet_max), Gw_p(1:n_phi_max)
 real(8) 	Rm(1:n_el,1:n_el),Rm_ocean(1:n_el,1:n_el),R11(1:n_tet_max,1:n_tet_max)
 real(8)	mu_mphi,tet_thresh_angl, mu_thresh

 integer i1,i2,i3, n_tet, n_phi
    ...
    if (land_percent .lt. land_percent_thresh) then 
!water		
		n_tet=30
		n_phi=40
	else
!land
	...
	mu_thresh=dcos(tet_thresh_angl*pi_dp/180.)

  	call gauss_l(n_tet,n_tet,low_lt,up_lt,mu(1:n_tet),Gw_t(1:n_tet))
 	call gauss_l(n_phi,n_phi,low_lp,up_lp,phi(1:n_phi),Gw_p(1:n_phi))

	R11=0.

  	Do i1=1, n_tet

   		If (mu(i1) .ge. mu_thresh) then
    		
			Do i2=1, i1
 			
				If (mu(i2) .ge. mu_thresh) then
        	
					Do i3=1, n_phi

                  		select case(nint(land_percent))
                  			case(0)
                     			call BRM_ocean(n_el,iBRM_water,n_par_water,mu(i1),mu(i2),(phi(i3)*deg_dp), &
                                    WAVE,x_vect_water(1:n_par_water),Rm)
                  			case(100)
                     			call BRM(n_el,iBRDF,iBPDF,n_par_land,n_par_land_i,mu(i1),mu(i2),(phi(i3)*deg_dp),&
                                    WAVE,x_vect_land(1:n_par_land),Rm)								
                  			case default
                     			If ((land_percent .lt. 0) .or. (land_percent .gt. 100)) then
                        			write(*,*) 'land_percent=',land_percent,' - unknown value'
                        			stop 'stop in BHRiso. "land_percent" is not correct'
                     			endif
                     			call BRM_ocean(n_el,iBRM_water,n_par_water,mu(i1),mu(i2),(phi(i3)*deg_dp), &
                                    WAVE,x_vect_water(1:n_par_water),Rm_ocean)
                     			call BRM(n_el,iBRDF,iBPDF,n_par_land,n_par_land_i,mu(i1),mu(i2),(phi(i3)*deg_dp),&
                  		...
                  		mu_mphi =Gw_p(i3)/pi_dp
                  		R11(i2,i1)=R11(i2,i1)+ mu_mphi*Rm(1,1)
!PL test						R11(i2,i1)=R11(i2,i1)+ mu_mphi
 	...
 	do i1 = 1, n_tet
    	do i2 = i1+1, n_tet
 			R11(i2,i1)=R11(i1,i2)
!			write(*,*) i1,i2,acos(mu(i1))*180./pi_dp, acos(mu(i2))*180./pi_dp, R11(i2,i1),  R11(i1,i2)
 		end do
	end do

!  calculation of isotropic bi-hemispherical reflectance 
    BHR_iso = 0.
	
	do i1 = 1, n_tet
		BHR_iso = BHR_iso + sum(R11(i1,1:n_tet)*mu(1:n_tet)*Gw_t(1:n_tet))*mu(i1)*Gw_t(i1)
    end do

	BHR_iso = 4.*BHR_iso
    
	if (BHR_iso .gt. 1.) BHR_iso=1.

 end subroutine BHRiso

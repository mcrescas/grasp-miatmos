...
#include "../../../constants_set/mod_globals.inc"
      ...
      implicit none
! --------------------------------------------------------------------------------
      character(len=GBL_FILE_PATH_LEN), intent(in)   ::  file_path_git_track
      character(len=GBL_FILE_PATH_LEN), intent(in)   ::  distname_O
      ...
      real      :: RCsum,pi2
      CHARACTER(len=GBL_FILE_PATH_LEN) :: full_name
! --------------------------------------------------------------------------------
...
! **           2 - surface area  mixture of spheroids           ** 
	...
	if ( (LB1 .gt. LE1) .or. (LE1 .gt. PHASE_MATRIX%DLSIN%NWL) .or. &
       (LB1 .gt. PHASE_MATRIX%DLSIN%NWL) ) then
...
!      write(*,*) 'after grid'
  ...
  if(PHASE_MATRIX%DLSIN%keyEL .gt. 0) then
    ...
    NEW_LINE('A'),'KM must be < or = KMpar'
    close(10)
    G_ERROR(trim(tmp_message))
    endif ! KM&KMpar
    ...
    enddo ! j
  elseif(PHASE_MATRIX%DLSIN%keyEL .eq. 0) then
      do j=1,PHASE_MATRIX%DLSIN%KM
      read(10,*)
      enddo ! j
  endif

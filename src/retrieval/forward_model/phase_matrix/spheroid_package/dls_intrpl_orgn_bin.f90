...
#include "../../../constants_set/mod_globals.inc"
      ...
      SUBROUTINE USMATRIX_bin(                       &
                              iu_main_output        & ! IN
                             ,IPRI_verbose          &
                             ,IPRI_additional_info  &
                             ,key,keyEL,keySUB,keyLS,key_RD1  &
                             ,key_RD                          &
...
!c      use alloc1
      ...
      use mod_ascending_order_array_check
      use mod_nodes_coincidence_check
      use mod_stop_report
...
! Validator for ratios
      status = ascending_order_array_check ( KR, R )
      if ( .not. status ) then
        write(str,*) KR
        fmt = '(a,a,'//trim(adjustl(str))//'f12.4)'
        write(tmp_message,fmt) &
        'settings ratio array is not in ascending order:', &
        NEW_LINE('A'), &
        R(1:KR)
        G_ERROR(trim(tmp_message))
      endif
      status = ascending_order_array_check ( XGRID1%NRATN, XGRID1%RATIO )
      if ( .not. status ) then
        write(str,*) XGRID1%NRATN
        fmt = '(2a,'//trim(adjustl(str))//'f12.4)'
        write(tmp_message,fmt) &
        'kernels ratio array is not in ascending order:', &
        NEW_LINE('A'), &
        XGRID1%RATIO(1:XGRID1%NRATN)
        G_ERROR(trim(tmp_message))
      endif
      status = nodes_coincidence_check ( XGRID1%NRATN, XGRID1%RATIO, KR, R )
      if ( .not. status ) then
        write(str,*) KR
        write(str1,*) XGRID1%NRATN
        fmt = '(4a,'//trim(adjustl(str))//'f12.4,3a,'//trim(adjustl(str1))//'f12.4)'
        write(tmp_message,fmt) &
        'settings ratios must be kernels nodes:', &
        NEW_LINE('A'), &
        'settings ratios:', &
        NEW_LINE('A'), &
        R(1:KR), &
...
goto 222
      IF(PO1S.LT.POS.OR.PO1S.GT.POF) THEN
        WRITE(tmp_message,'(a,a,a,i0,3(2x,a,es11.4))') &
...
222 continue
      IF(PO1F.LT.POS.OR.PO1F.GT.POF) THEN
        WRITE(tmp_message,'(a,a,a,es11.4,3(2x,a,es11.4))') &
        'given size parameter is out of look-up table (in USMATRIX 2)', &
        NEW_LINE('A'), &
        'wl =',WL(L),'wl_base =',WAVEL,'given maximum size parameter =',PO1F,'kernel size parameter range is from',POS,'to',POF
        G_ERROR(trim(tmp_message))
      ENDIF
...
! PL 2022-01-05
      ...
      IF(PO1S.LT.POS.OR.PO1S.GT.POF) THEN
        ...
        'given size parameter is out of look-up table (in USMATRIX 1)'
        WRITE(iu_main_output,'(5(a,es11.4,2x))') &
        'wl =',WL(L),'wl_base =',WAVEL,'given minimum size parameter =',PO1S,'kernel size parameter range is from',POS,'to',POF
        write(iu_main_output,'(2(a,es11.4,2x))') 'given size parameters less than',PO1S, &
        'will be replaced with minimum size parameter node =',POS
      ENDIF
      !IF(PO1F.LT.POS.OR.PO1F.GT.POF) THEN
        !WRITE(iu_main_output,'(a)') &
        !'given size parameter is out of look-up table in USMATRIX 2'
      ...
      if(key.eq.0) then
         if(key_RD1.eq.1) then
            call USU_LS_bin(            &
                  ...
                  ,IPRI_verbose         &
                  ,IPRI_additional_info &
                  ,key_RD,keyEL,keySUB,keyLS,KR,R,RD,RATIO,NRATN  &
...
!c **************************************************************** 
      SUBROUTINE USU_LS_bin(                  &
                         iu_main_output       &
                        ,IPRI_verbose         &
                        ,IPRI_additional_info &
                        ,key_RD,keyEL,keySUB,keyLS,KR,R,RD,RATIO     &
...
! PL 2022-01-05
      PO_min_kernels = grid1(1) * PI2 / WAVEL
      IF ( PO .LT. PO_min_kernels ) THEN
        IF ( IPRI_additional_info ) THEN
        write(iu_main_output,'(2(a,es11.4,2x))') 'current size parameter =',PO, &
        'replaced with minimum size parameter kernek node =',PO_min_kernels
        ENDIF
        PO = PO_min_kernels
      ENDIF
! PL

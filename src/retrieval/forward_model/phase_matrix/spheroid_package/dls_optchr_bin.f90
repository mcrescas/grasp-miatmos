...
!write(*,*) 'grid:'
!write(*,'(10es12.4)') grid(1:KN)
!write(*,*) '(R(i),RD(i),i=1,KR):'
!write(*,*) (R(i),RD(i),i=1,KR)
...
!write(*,*) pomin,pomax,xnmin,xnmax,xkmin,xkmax,'  - pomin,pomax,xnmin,xnmax,xkmin,xkmax'
      ...
      if ( error_present() ) return
         ...
         !write(*,*) 'in OPTCHAR11: US11=',US11(1,1,1),' US11=',US11(KM,KN,NWL)
!c
...
goto 33
! no decision yet for the part below
      if((coeff1 - 1.0)/coeff1 .gt. 1e-3) then
        write(tmp_message,'(2(a,es11.4,2x),a,i4,2es12.4,3a)') &
        'dlnr =',dlnr,'dlnr1 =',dlnr1, &
        NEW_LINE('A'), &
        KN,grid(1),grid(KN),'  - KN,grid(1),grid(KN)', &
        NEW_LINE('A'), &
        'dlnr (settings) has be <=  dlnr1 (kernels)'
...
!	  write(*,'(10es12.4)') US22(1,1:KN,1)
!      write(*,*) 'test DLS_optchr_bin.f SD(1,1:KN) :'
	  !write(*,'(10es12.4)') SD(1,1:KN)
	  !write(*,*) 'LB=',LB,' LE=',LE
      ...
      !write(*,*) 'dlnr=',dlnr,'  coeff =',coeff
      !write(*,*) 'in optchar_bin SD: '
      !write(*,'(10es12.4)') SD(1,1:KN)
      !write(*,*) 'in optchar_bin USEA: '
      !write(*,'(10es12.4)') USEA(1,1:KN,L)
      !write(*,*) 'in optchar_bin USEA 2: '
      !write(*,'(10es12.4)') USEA(2,1:KN,L)
      !write(*,*) 'in optchar_bin US11: '
      !write(*,'(10es12.4)') US11(1,1:KN,L)
      !write(*,*) 'in optchar_bin USEA 1: '
      !write(*,'(10es12.4)') USEA(1,1:KN,L)
      !write(*,*) 'in optchar_bin USEA 2: '
      !write(*,'(10es12.4)') USEA(2,1:KN,L)
      ENDDO ! L
...
!c ** Check f11 norma
!c
      !if ( KM .eq. 180 .or. KM .eq. 181 ) then
      if ( KM .ge. 35 ) then
      if ( ANGLE(1) .eq. 0. .and. ANGLE(KM) .eq. 180. ) then
        if ( keySUB .eq. 0 ) then
          if ( keyEL .gt. 0 ) then
          do L=LB,LE
            call SINT  ( ANGLE(1:KM), f11(1:KM,L), KM, xnorm(L) )
            call ASYPAR( ANGLE(1:KM), f11(1:KM,L), KM, g(L) )
!write(*,*) 'test test L =',L,'  xnorm=',xnorm(L),'  xsca=',xsca(L)
!stop 'test stop in DLS_optchr_bin'
          enddo ! L
          endif
          if ( keyEL .gt. 1 ) then
          do L=LB,LE
            f12(1:KM,L) = -f12(1:KM,L) / xnorm(L) / f11(1:KM,L)
          enddo ! L
          endif
          if ( keyEL .gt. 2 ) then
          do L=LB,LE
            f22(1:KM,L) = f22(1:KM,L) / xnorm(L) / f11(1:KM,L)
          enddo ! L
          endif
          if ( keyEL .gt. 3 ) then
          do L=LB,LE
            f33(1:KM,L) = f33(1:KM,L) / xnorm(L) / f11(1:KM,L)
          enddo ! L
          endif
          if ( keyEL .gt. 4 ) then
          do L=LB,LE
            f34(1:KM,L) = f34(1:KM,L) / xnorm(L) / f11(1:KM,L)
          enddo ! L
          endif
          if ( keyEL .gt. 5 ) then
          do L=LB,LE
            f44(1:KM,L) = f44(1:KM,L) / xnorm(L) / f11(1:KM,L)
          enddo ! L
          endif
        else ! keySUB=1
          if ( keyEL .gt. 0 ) then
...
!stop 'test stop in DLS_optchr_bin'
          ...
          if ( keyEL .gt. 1 ) then
          do L=LB,LE
            f12(1:KM,L) = f12(1:KM,L) / xnorm(L) * xsca(L)
          enddo ! L
          endif
          if ( keyEL .gt. 2 ) then
          do L=LB,LE
            f22(1:KM,L) = f22(1:KM,L) / xnorm(L) * xsca(L)
          enddo ! L
          endif
          if ( keyEL .gt. 3 ) then
          do L=LB,LE
            f33(1:KM,L) = f33(1:KM,L) / xnorm(L) * xsca(L)
          enddo ! L
          endif
          if ( keyEL .gt. 4 ) then
          do L=LB,LE
            f34(1:KM,L) = f34(1:KM,L) / xnorm(L) * xsca(L)
          enddo ! L
          endif
          if ( keyEL .gt. 5 ) then
          do L=LB,LE
            f44(1:KM,L) = f44(1:KM,L) / xnorm(L) * xsca(L)
          enddo ! L
          endif
        endif ! keySUB .eq. 0
      endif ! ANGLE(1) .eq. 0. .and.
      endif ! KM .ge. 35
      if ( key .eq. 0 ) then
       SD(1,1:KN)=SD(1,1:KN)/coeff1 !/coeff
	    !else 

...
//    grasp_controller_read_settings(argc,argv,&settings, YS_PARSE_SETTINGS_FILE_MANDATORY);
//    
...
//    
//    CU_ASSERT_STRING_EQUAL("2015-03-23T05_00_00Z", grasp_output_stream_filename_generate_by_tile_from("tile_from", NULL, settings, segment, output, &tile_description.dimensions, 1,1,1)); 
...
//    
//    CU_ASSERT_STRING_EQUAL("2015-03-23T06_00_00Z", grasp_output_stream_filename_generate_by_tile_to("tile_to", NULL, settings, segment, output, &tile_description.dimensions, 1,1,1)); 
...
//    
//    // 5) Clean up 
//    grasp_controller_clean_memory(settings, &tile_description, NULL, &functions, NULL);
//    free(segment);
//    free(output);
//    free(results.segment_result);  // it should be used trackmem_free but for a tests it is not mandatory
//    free(results.tile_result_map); // it should be used trackmem_free but for a tests it is not mandatory
}

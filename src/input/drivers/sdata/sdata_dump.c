...
char *g_appname;
char *g_appdir;
char g_version[CSTRING_MAXLEN + 1] = "0.2.0";
char g_output_file[CSTRING_MAXLEN + 1] = "";
uint32_t g_flags = OPT_UNFOLD; /* C99 only */
...
void usage() {
  ...
  fprintf(stderr, 
	  ...
	  "  -R                  displays sdata in raw, but normalized format\n"
	  "  -U                  unfold sdata (default)\n");
  exit (EXIT_FAILURE);
}
...
void unfold_sdata(const char *file_name) {
  ...
  err = sdata_open(file_name, &handle);
  if (err!=0){
      exit (EXIT_FAILURE);
  }
  ...
  err = sdata_load_box(handle, NULL, &box /* settings inferred from the SDATA file itself */);
  if (err!=0){
      exit (EXIT_FAILURE);
  }
  if (box == NULL) {

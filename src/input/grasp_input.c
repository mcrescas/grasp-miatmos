...
int grasp_input_initialize_tile_description(int nfiles, char input_files[GRASP_INPUT_MAX_FILES][_GBL_FILE_PATH_LEN], grasp_tile_description_t *tile_description){   
    ...
    tile_description->ninput_files=grasp_input_expand_files(nfiles, input_files,&(tile_description->input_files));  
    ...
    if(tile_description->ninput_files==0){
        ...
        return GRASP_ERROR_INPUT_NO_INPUT_FILES;        
    }   
...
int grasp_input_position_of_inversion(const grasp_tile_dimensions_t *tile_dimensions, int id_inversion, int *icol, int *irow, int *itime){
    ...
    maximums[1]=tile_dimensions->segment_ncols;
    maximums[2]=tile_dimensions->segment_nrows;
...
int grasp_input_extract_segment(grasp_settings *settings, grasp_input_driver_t *driver, int ntransformers,grasp_input_transformer_t *transformers, grasp_segment_t *segment, grasp_results_t *results, grasp_tile_dimensions_t *tile_dimensions, int id_inversion){
    ...
    if(npixels<0) return GRASP_ERROR_DRIVER+npixels;

    // Dumping SDATA file (for debuggin purposes)
    grasp_output_stream_initialize(settings->input.sdata_dump_after_driver, &grasp_stream);
    ...
    if (grasp_output_stream_writable(&grasp_stream)) {
        ...
        grasp_input_print_sdata(f, "segment", &(segment->sdata));
    }
    grasp_output_stream_close(&grasp_stream);
    grasp_output_stream_destroy(&grasp_stream);
    ...
    for (i = 0; i < ntransformers; i++) {                 
        status=transformers[i].function(settings,segment);
    ...
    // Dumping SDATA file (for debuggin purposes)
    grasp_output_stream_initialize(settings->input.sdata_dump_after_transformer, &grasp_stream);
    f = grasp_output_stream_open(&grasp_stream, settings, segment, NULL, tile_dimensions, icol, irow, itime);
    if (grasp_output_stream_writable(&grasp_stream)) {
        grasp_input_dump_sdata(f, &(segment->sdata));
    }
...
int grasp_input_get_num_pixels_at_it(const sensor_data_t *sdata, int it) {
  ...
  assert(sdata != NULL);
  npixels = sdata->npixels;

  assert(1 <= it && it <= sdata->nt);

  npixels_it = 0;
  
  for (ipixel = 0 ; ipixel < npixels ; ipixel++) {
    const pixel_t *segment_pixel = &sdata->pixel[ipixel];
  ...
  assert(npixels_it <= sdata->nx*sdata->ny);
	...
	for(ivm=0 ; ivm<owl.nbvm[ip] ; ivm++) {
	  ...
	  case (MEAS_TYPE_TOD):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                ...
                grasp_segment_set_pixel_meas_tod(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_AOD):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_aod(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_ABS):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_aaod(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_HTOD):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_htod(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  ...
	  case (MEAS_TYPE_P11_rel_ang):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_p11_rel_ang(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P12_rel):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_p12_rel(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  ...
	  case (MEAS_TYPE_DPAR):
		...
		assert(0 <= ivm_valid && ivm_valid < _KVERTM);
		assert(is_valid_value(owl.mprof[ip][ivm], missing_value));
		sdata->pixel[ipixel].meas[iwl].mprof[ip][ivm_valid]=owl.mprof[ip][ivm];
		sdata->pixel[ipixel].meas[iwl].dpar[ivm_valid]=value;
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_DPER):
		...
		assert(is_valid_value(owl.mprof[ip][ivm], missing_value));
		sdata->pixel[ipixel].meas[iwl].mprof[ip][ivm_valid]=owl.mprof[ip][ivm];
		sdata->pixel[ipixel].meas[iwl].dper[ivm_valid]=value;
		num_good_meas[ip]++;
	      }
	    }
	    break;
      ...
      case (MEAS_TYPE_VBS):
          {
              ...
              if(is_valid_value(value, missing_value)) {
                  ...
                  assert(is_valid_value(owl.mprof[ip][ivm], missing_value));
                  grasp_segment_set_pixel_meas_mprof(sdata,ipixel,iwl,ip,ivm_valid,owl.mprof[ip][ivm]);
                  grasp_segment_set_pixel_meas_vbs(sdata,ipixel,iwl,ivm_valid,value);
                  num_good_meas[ip]++;
              }
          }
          break;
	  case (MEAS_TYPE_VEXT):
		...
		assert(is_valid_value(owl.mprof[ip][ivm], missing_value));
		sdata->pixel[ipixel].meas[iwl].mprof[ip][ivm_valid]=owl.mprof[ip][ivm];
		sdata->pixel[ipixel].meas[iwl].vext[ivm_valid]=value;
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  ...
	  case (MEAS_TYPE_I_rel_sum):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_i_rel_sum(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P_rel):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_p_rel(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P11_intd):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);               
                grasp_segment_set_pixel_meas_p11_intd(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P11_intd_cut_off_1):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);
                grasp_segment_set_pixel_meas_p11_intd_cut_off_1(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P11_intd_cut_off_2):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);
                grasp_segment_set_pixel_meas_p11_intd_cut_off_2(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P11_intd_cut_off_3):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);
                grasp_segment_set_pixel_meas_p11_intd_cut_off_3(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
	  case (MEAS_TYPE_P11_intd_cut_off_4):
		...
		assert(is_valid_value(owl.phi[ip][ivm], missing_value));
                grasp_segment_set_pixel_meas_thetav(sdata,ipixel,iwl,ip,ivm_valid,owl.thetav[ip][ivm]);
                grasp_segment_set_pixel_meas_phi(sdata,ipixel,iwl,ip,ivm_valid,owl.phi[ip][ivm]);
                grasp_segment_set_pixel_meas_p11_intd_cut_off_4(sdata,ipixel,iwl,ivm_valid,value);
		num_good_meas[ip]++;
	      }
	    }
	    break;
...
static void convert_segment_pixel_to_sdata_pixel(SDATA_PIXEL *sdata_pixel, const pixel_t *segment_pixel) {
  ...
  for (iwl = 0 ; iwl < segment_pixel->nwl ; iwl++) {
    ...
    for (ip = 0 ; ip < nip ; ip++) {
      ...
      if (meas_type == MEAS_TYPE_LS || meas_type == MEAS_TYPE_DP || meas_type == MEAS_TYPE_RL || meas_type == MEAS_TYPE_VBS) {
	...
	switch (meas_type) {
	case MEAS_TYPE_TOD:
	  sdata_pixel->tod[iwl][ivm] = meas->tod[ivm];
	  break;
	case MEAS_TYPE_AOD:
	  sdata_pixel->aod[iwl][ivm] = meas->aod[ivm];
	  break;          
	case MEAS_TYPE_ABS:
	  sdata_pixel->aaod[iwl][ivm] = meas->aaod[ivm];
	  break;
	case MEAS_TYPE_HTOD:
	  sdata_pixel->htod[iwl][ivm] = meas->htod[ivm];
	  break;
	case MEAS_TYPE_P11:
	...
	case MEAS_TYPE_P44:
	  ...
	  break;
	case MEAS_TYPE_P11_rel_ang:
	  sdata_pixel->p11_rel_ang[iwl][ivm] = meas->p11_rel_ang[ivm];
	  break;
	case MEAS_TYPE_P12_rel:
	  sdata_pixel->p12_rel[iwl][ivm] = meas->p12_rel[ivm];
	  break;
	case MEAS_TYPE_P11_intd:
	  sdata_pixel->p11_intd[iwl][ivm] = meas->p11_intd[ivm];
	  break;
	case MEAS_TYPE_LS:
	...
	case MEAS_TYPE_DP:
	  ...
	  break;
	case MEAS_TYPE_DPER:
	  sdata_pixel->DPER[iwl][ivm] = meas->dper[ivm];
	  break;
	case MEAS_TYPE_DPAR:
	  sdata_pixel->DPAR[iwl][ivm] = meas->dpar[ivm];
	  break;
	case MEAS_TYPE_RL:
	  sdata_pixel->RL[iwl][ivm] = meas->rl[ivm];
	  break;
    case MEAS_TYPE_VBS:
      sdata_pixel->VBS[iwl][ivm] = meas->vbs[ivm];
      break;
	case MEAS_TYPE_VEXT:
	  sdata_pixel->VEXT[iwl][ivm] = meas->vext[ivm];
	  break;
	case MEAS_TYPE_I:
	...
	case MEAS_TYPE_P:
	  ...
	  break;
	case MEAS_TYPE_I_rel_sum:
	  sdata_pixel->I_rel_sum[iwl][ivm] = meas->i_rel_sum[ivm];
	  break;
	case MEAS_TYPE_P_rel:
	  sdata_pixel->P_rel[iwl][ivm] = meas->p_rel[ivm];
	  break;
    case MEAS_TYPE_P11_intd_cut_off_1:
        sdata_pixel->p11_intd_cut_off_1[iwl][ivm] = meas->p11_intd_cut_off_1[ivm];
        break;
    case MEAS_TYPE_P11_intd_cut_off_2:
        sdata_pixel->p11_intd_cut_off_2[iwl][ivm] = meas->p11_intd_cut_off_2[ivm];
        break;
    case MEAS_TYPE_P11_intd_cut_off_3:
        sdata_pixel->p11_intd_cut_off_3[iwl][ivm] = meas->p11_intd_cut_off_3[ivm];
        break;
    case MEAS_TYPE_P11_intd_cut_off_4:
        sdata_pixel->p11_intd_cut_off_4[iwl][ivm] = meas->p11_intd_cut_off_4[ivm];
        break;
	default:
...
#warning "__SDATA_VERSION__ duplicated"
  ...
  for (ipixel = 0 ; ipixel < sdata->npixels ; ipixel++) {
    ...
    SDATA_RECORD* record = malloc(sizeof(SDATA_RECORD));
    if (segment_pixel->it != previous_it) {      
      ...
      if (segment_pixel->it != previous_it + 1) {
        grasp_input_print_sdata(stderr, "segment", sdata);
      ...
      record->npixels = grasp_input_get_num_pixels_at_it(sdata, segment_pixel->it);
...
void grasp_input_print_segment(FILE *output_stream, const char *label, const grasp_segment_t *segment) {
  int ipixel;
  grasp_input_print_sdata(output_stream, label, &segment->sdata);
  for (ipixel = 0 ; ipixel < segment->sdata.npixels ; ipixel++) {
    grasp_input_print_segment_pixel(output_stream, segment, ipixel);
  }
}
...
void grasp_input_print_segment_pixel(FILE *output_stream, const grasp_segment_t *segment, size_t ipixel) {
  ...
  sprintf(pixel_label, "pixel %lu", ipixel + 1);
  convert_segment_pixel_to_sdata_pixel(&sdata_pixel, segment_pixel);
...
void grasp_input_print_sdata(FILE *output_stream, const char *label, const sensor_data_t *sdata) {
  ...
  fprintf(output_stream, "sdata->npixels: %d\n", sdata->npixels);
  fprintf(output_stream, "sdata->NX: %d\n", sdata->nx);
  fprintf(output_stream, "sdata->NY: %d\n", sdata->ny);
  fprintf(output_stream, "sdata->NT: %d\n", sdata->nt);
  fprintf(output_stream, "\n");
  for (ipixel = 0 ; ipixel < sdata->npixels ; ipixel++) {
    const pixel_t *segment_pixel = &sdata->pixel[ipixel];
    SDATA_PIXEL sdata_pixel;
...
int grasp_input_expand_files(int nfiles, char input_files[GRASP_INPUT_MAX_FILES][_GBL_FILE_PATH_LEN], char ***output_files){
    ...
    for (i = 0; i < nfiles; i++) {
        nfiles_expanded[i]=find_files_from_pattern(input_files[i],&glob_expanded);

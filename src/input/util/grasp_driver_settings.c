 ...
 * @author Fabrice Ducos <fabrice.ducos@univ-lille.fr>
 *
 */
...
bool grasp_driver_settings_init_by_row_col(grasp_driver_settings_t *settings, int nfiles, char **files, double row_min, double col_min, 
  ...
  if (err != 0) {
    fprintf(DEBUG_STREAM, "%s:%d: failed at convert_string_to_time(str_gmt_time_min = \"%s\", &gmt_time_min, TIMEFMT_ISO8601). Make sure the time is specified as \"YYYY-MM-DDThh:mm:ssZ\"\n",
  ...
  if (err != 0) {
    fprintf(DEBUG_STREAM, "%s:%d: failed at convert_string_to_time(str_gmt_time_max = \"%s\", &gmt_time_max, TIMEFMT_ISO8601). Make sure the time is specified as \"YYYY-MM-DDThh:mm:ssZ\"\n",

...
void yamlsettings_initialize_dictionary(yamlsettings_dictionary_t *dictionary, const char *inputFile, yamlsettings_parser_settings_file_mode settings_file_mode){
    int i,j;
    char *ptr;
    char *tmp;
    ...
    if(settings_file_mode==YS_PARSE_SETTINGS_INLINE){
        // In this case we can use getcwd or if the string starts by !, substract the path between ! and ; symbols.
        if(inputFile[0]=='!'){//subtract path
            // Extract the path
            for (i = 1; i < strlen(inputFile); i++) {
                if(inputFile[i]==';'){
                    break;
                }
            }
            assert(i>1 && i<strlen(inputFile));
            tmp=(char *)malloc(sizeof(char)*(i+3));
            strncpy(tmp,&inputFile[1],i-1);
            tmp[i-1]='\0';
            
            // It can be relative or absolute. If it is relative we transform it in absolute by starting from cwd
            if(inputFile[1]=='/'){ //Absolute path
                // Just copy the extracted path
                dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION] = (char *) malloc(sizeof (char)*(strlen(tmp)+5));
                strcpy(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],tmp);
            }else{ //Relative path
                // Copy concatenated CWD + extracted path.
                dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION] = (char *) malloc(sizeof (char)*(strlen(dictionary->files[YAMLSETTINGS_FILE_COMMAND_LINE])+strlen(tmp)+5));
                strcpy(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],dictionary->files[YAMLSETTINGS_FILE_COMMAND_LINE]);
                strcat(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],"/");
                strcat(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],tmp);
            }
            
            free(tmp);
        }else{ // We use getcwd
            dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION] = (char *) malloc((sizeof (char)*(strlen(dictionary->files[YAMLSETTINGS_FILE_COMMAND_LINE])+100)));
            assert(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION]!=NULL);
            strcpy(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],dictionary->files[YAMLSETTINGS_FILE_COMMAND_LINE]);
            strcat(dictionary->files[YAMLSETTINGS_FILE_MAIN_CONFIGURATION],"/inline-tmp.yml");
        }
    }else{
    ...
    // In command line we finish adding the executable name
    strcat(dictionary->files[YAMLSETTINGS_FILE_COMMAND_LINE],"/grasp");
...
bool yamlsettings_parameter_is_unset(int param_index, yamlsettings_dictionary_t *dictionary){
    ...
    for (i = 0; i < yamlsettings_parameter_number_of_elements(&dictionary->parameters[param_index]); i++) {
        if(dictionary->parameters[param_index].settings_file != NULL && dictionary->parameters[param_index].settings_file[i] != dictionary->files[YAMLSETTINGS_FILE_UNSET]){
            return false;
        }
    }
...
int yamlsettings_parameter_nelements_set(int param_index, yamlsettings_dictionary_t *dictionary){
    int i, c=0;
 ...
 */
void yamlsettings_dictionary_dump_print_indentation(char *r, int level) {
    int i;
...
int yamlsettings_dictionary_dump_recursive(char *r, yamlsettings_dictionary_t *dictionary,int level, int iparameterstart, int indexes[YAMLSETTINGS_MAX_PARAMETER_DIMENSIONS], int iparameterstop, bool print_defaults, bool force_arrays){
    ...
    for (iparameter = iparameterstart; iparameter < iparameterstop; iparameter++) {
        ...
        if(title==NULL){ // This means that we have finish of exploring this level of blocks
            if(iparameter+1<iparameterstop){ 
               ...
               free(tmp1);
               next_level--;
               yamlsettings_dictionary_dump_recursive(r, dictionary,next_level,iparameter+1, indexes,iparameterstop, print_defaults, force_arrays);  
            }
...
//            free(title);
            ...
            if(nelements>0){
                for (j = 0; j < nelements; j++){ 
                    if(print_defaults==true || yamlsetting_parameter_is_not_omissible(dictionary, iparameter, level)==true){
                        ...
                        yamlsettings_dictionary_dump_print_indentation(r, level);   
                        sprintf(r,"%s%s[%d]:\n",r, tmp, j+1);
                        // fprintf(f, "%s[%d]:\n", tmp, j+1); 
                    }

                    yamlsettings_dictionary_dump_recursive(r, dictionary,level+1,iparameterstart,indexes, iparameter+next_indexstop, print_defaults, force_arrays);
                    indexes[nindexes-1]++;
                }  
            }else{
           ...
           level=next_level-1;
            ...
            if(print_defaults==true || yamlsetting_parameter_is_not_omissible(dictionary, iparameter, level)==true){  //here we can filter if we print only set values
                ...
                if(level+1==yamlsettings_parameter_number_of_blocks(dictionary->parameters[iparameter].name)){
                    if(dictionary->parameters[iparameter].allow_array==YS_PARAM_ARRAY || (dictionary->parameters[iparameter].allow_array==YS_PARAM_OPTIONAL_ARRAY && force_arrays) || (dictionary->parameters[iparameter].allow_array==YS_PARAM_OPTIONAL_ARRAY && dictionary->parameters[iparameter].counter_mem_pos[
                        indexND(
                        ...
                        indexND(
                            ...
                            tmp=yamlsettings_data_type_get_string(dictionary->parameters[iparameter].data_type_get, dictionary->parameters[iparameter].mem_pos, 
                                    indexND(
                                        ...
                                        indexes,
                                        dictionary->parameters[iparameter].dimensions.dimension_size)                    
                                    , dictionary->parameters[iparameter].maxlength, true);
                            sprintf(r, "%s%s",r, tmp); 
                            //fprintf(f, "%s", tmp); 
                            free(tmp);                        
                        }
        ...
        yamlsettings_dictionary_dump_recursive(r, dictionary,level,iparameterstart,indexes,iparameterstop, print_defaults, force_arrays);

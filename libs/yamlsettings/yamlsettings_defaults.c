...
#include <grasp/utils.h> // safe_atoi 
...
int ys_d_int_array_increasing (int param_index, yamlsettings_parameter *parameter, yamlsettings_status *status){
    int i,initial,result;
    char value_to_set[10];
    
    result=safe_atoi(parameter->default_value.arguments[0], &initial);
    
    if(result!=0){
        printf("ERROR: ys_d_int_array_increasing base number of increasing set is not a valid integer %s\n", parameter->default_value.arguments[0]);
        abort();
    }
   
    for(i=0; i<yamlsettings_parameter_number_of_elements(parameter); i++){ // Initilize values. Loop for initilize arrays
        sprintf(value_to_set,"%d",initial+i);
        if(yamlsettings_data_type_assign(parameter->data_type_set, status, parameter->mem_pos,yamlsettings_parameter_number_of_elements(parameter),parameter->name,  value_to_set, i, parameter->maxlength, YAMLSETTINGS_FILE_UNSET, parameter->settings_file[0])<0){    
            printf("ERROR: ys_d_int_array_increasing: Invalid initial value of parameter %s\n", parameter->name);
            abort();
        }
    }    

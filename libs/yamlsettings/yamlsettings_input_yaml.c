...
int yamlsettings_input_yaml_read(yamlsettings_status *status, GNode **cfg, int nparameters, char const *parameters[], char *files[YAMLSETTINGS_MAXIMPORTEDFILES], yamlsettings_parser_mode file_modes[YAMLSETTINGS_MAXIMPORTEDFILES], yamlsettings_parser_settings_file_mode settings_file_mode) {
    ...
    *cfg = yamlsettings_create_node(VAR, 0, 0, YAMLSETTINGS_FILE_MAIN_CONFIGURATION, "");
    ...
    if(settings_file_mode!=YS_PARSE_SETTINGS_FILE_FORBIDDEN){
        ...
        while (files[i] != NULL) {
            if (strcmp(files[i], "-") != 0 && (settings_file_mode!=YS_PARSE_SETTINGS_INLINE || (settings_file_mode==YS_PARSE_SETTINGS_INLINE && i !=YAMLSETTINGS_FILE_MAIN_CONFIGURATION))) {
            ...
            if (!source) {
                ...
                if(settings_file_mode==YS_PARSE_SETTINGS_INLINE){
                    j=-1;
                    if(parameters[0][0]=='!'){ // We have to ignore the path
                        for (j = 1; j < strlen(parameters[0]); j++) {
                            if(parameters[0][j]==';'){
                                break;
                            }
                        }
                    }
                    j++; // position of first character after the magic wildcard that represent the path (between ! and ;)
                    yaml_parser_set_input_string(&parser,(const unsigned char *)&parameters[0][j],  (strlen(parameters[0])-j)*sizeof(char));
                }else{

...
typedef struct yamlsettings_find_usages_struct_{
    int nuses;
    GNode *uses[1000];
    char *searched_name;
    int nchars;
}yamlsettings_find_usages_struct;

gboolean yamlsettings_find_usages(GNode *node, gpointer data){
    yamlsettings_find_usages_struct *indata;
    char *node_name;
    
    indata=(yamlsettings_find_usages_struct *)data;
    
    assert (indata->nuses<1000);
    
    node_name=((yamlsettings_node_element *)node->data)->data;
    
    if(strncmp(node_name, indata->searched_name, indata->nchars)==0){
        indata->uses[indata->nuses]=node;
...
typedef struct yamlsettings_check_repeated_variable_struct_{
    bool repeated;
    char *searched_name;
}yamlsettings_check_repeated_variable_struct;


void yamlsettings_check_repeated_variable(GNode *node, gpointer data){
    yamlsettings_check_repeated_variable_struct *data_check_repeated;
    
    data_check_repeated=(yamlsettings_check_repeated_variable_struct *)data;
    
    if(strcmp(((yamlsettings_node_element *)node->data)->data,data_check_repeated->searched_name)==0){
        data_check_repeated->repeated=true;
    }
}

typedef struct yamlsettings_find_replace_remove_variables_struct_{
    yamlsettings_status *status;
    GNode *root;
    GNode *variables;
}yamlsettings_find_replace_remove_variables_struct;

void yamlsettings_find_replace_variables(GNode *node, yamlsettings_find_usages_struct *data_find_usages){
    int i,j;
    yamlsettings_node_element *node_data;
    GNode *to_be_replaced, *parent, *variable_children;
    int type, position;

    for(i=0; i<data_find_usages->nuses; i++){
        to_be_replaced=data_find_usages->uses[i];
        parent=to_be_replaced->parent;
        type=((yamlsettings_node_element *)to_be_replaced->data)->type;
        position=((yamlsettings_node_element *)to_be_replaced->data)->element_position;
        g_node_unlink(to_be_replaced);
        yamlsettings_destroy_node_data(to_be_replaced, NULL);
        g_node_destroy(to_be_replaced); 
        
        if(type==VAL){
            ((yamlsettings_node_element *)parent->data)->type = ((yamlsettings_node_element *)node->data)->type;
            ((yamlsettings_node_element *)parent->data)->nelements = ((yamlsettings_node_element *)node->data)->nelements;
            variable_children=node->children;

            while(variable_children!=NULL){
                node_data=(yamlsettings_node_element *)malloc(sizeof(yamlsettings_node_element));
                node_data->type=((yamlsettings_node_element *)variable_children->data)->type;
                node_data->nelements=((yamlsettings_node_element *)variable_children->data)->nelements;
                node_data->element_position=((yamlsettings_node_element *)variable_children->data)->element_position;
                node_data->file_index=((yamlsettings_node_element *)variable_children->data)->file_index;
                node_data->data=(char *)malloc(sizeof(char)*(strlen(((yamlsettings_node_element *)variable_children->data)->data)+1));
                strcpy(node_data->data,((yamlsettings_node_element *)variable_children->data)->data);

                g_node_append(parent, g_node_new((gpointer)node_data));


                variable_children=variable_children->next;
            }
        }
        if(type==SEQ){
            ((yamlsettings_node_element *)parent->data)->nelements += ((yamlsettings_node_element *)node->data)->nelements - 1;
            variable_children=g_node_last_child(node); // node->children;

            while(variable_children!=NULL){
                node_data=(yamlsettings_node_element *)malloc(sizeof(yamlsettings_node_element));
                node_data->type=SEQ; 
                node_data->nelements=0;
                node_data->element_position=position;
                node_data->file_index=((yamlsettings_node_element *)variable_children->data)->file_index;
                node_data->data=(char *)malloc(sizeof(char)*(strlen(((yamlsettings_node_element *)variable_children->data)->data)+1));
                strcpy(node_data->data,((yamlsettings_node_element *)variable_children->data)->data);

                g_node_insert_data(parent, position, (gpointer)node_data);

                variable_children=variable_children->prev;
            }
            // Fix index of positions
            variable_children=parent->children;
            j=0;
            while(variable_children!=NULL){
                ((yamlsettings_node_element *)variable_children->data)->element_position=j;
                j++;
                variable_children=variable_children->next;
            }
        }
...
bool yamlsettings_is_variable(char *name){
    int char_len;

    char_len = strlen(name);
    
    if (char_len>1 && name[0] == '^' ){
        // it can be a variable or a function.
        if(
                (char_len>12 && strncmp(name,"^expand(",8)==0  && name[char_len-1] == ')' && name[char_len-2] == ')') ||
                (char_len>13 && strncmp(name,"^repeat(",8)==0 && name[char_len-1] == ')' && name[char_len-2] == ')') 
           ){
            return false;
        }else{
            return true;
...
void yamlsettings_find_replace_remove_variables(GNode *node, gpointer data){
    char *name;
    GNode *variables, *root;
    yamlsettings_find_replace_remove_variables_struct *indata;
    yamlsettings_status *status;
    char error [512];
    yamlsettings_check_repeated_variable_struct data_check_repeated;
    yamlsettings_find_usages_struct data_find_usages;
    
    indata=(yamlsettings_find_replace_remove_variables_struct *)data;
    variables=(GNode *)(indata->variables);
    root=(GNode *)(indata->root);
    status=indata->status;
    
    name=((yamlsettings_node_element *)node->data)->data;
    
    
    if (yamlsettings_is_variable(name)){
        g_node_unlink(node);
        
        // Check if repeated
        data_check_repeated.repeated=false;
        data_check_repeated.searched_name=name;
        g_node_children_foreach (variables, G_TRAVERSE_ALL, yamlsettings_check_repeated_variable, (gpointer)(&data_check_repeated));
        if(data_check_repeated.repeated==true){
            sprintf(error,"Variable %s is defined several times", name);
            yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)node->data)->file_index, YS_ERROR);
        }
        
        // Find usages 
        data_find_usages.nuses=0;
        data_find_usages.searched_name=name;
        data_find_usages.nchars=9999;
        g_node_traverse(root, G_LEVEL_ORDER, G_TRAVERSE_LEAVES, -1, yamlsettings_find_usages, (gpointer)&data_find_usages);
        
        // Replace every ocurrences
        //fprintf(stderr, "%d usages of %s\n", data_find_usages.nuses, name);
        yamlsettings_find_replace_variables(node, &data_find_usages);
        
        g_node_append(variables, node);
        
        // fprintf(stderr, "DEBUG: %s: %d %s  \n", __FILE__, __LINE__, ((yamlsettings_node_element *)node->data)->data);
    }
}

char *yamlsettings_get_variable_value(GNode *variables, char *variable_name, GNode *parent, char *value_read, yamlsettings_status *status){
    GNode *variable_children;
    char error[512];
    char *tmp;
    variable_children=variables->children;
    
    while(variable_children!=NULL){
        if(strcmp(((yamlsettings_node_element *)variable_children->data)->data, variable_name)==0){
            break;
        }
        variable_children=variable_children->next;
    }
    
    if(variable_children!=NULL){
        variable_children=variable_children->children;
        
        if(((yamlsettings_node_element *)variable_children->data)->type==SEQ){
            sprintf(error,"A function cannot use an array as argument. It happens in the node %s with the value %s", 
                    ((yamlsettings_node_element *)parent->data)->data,
                    value_read);
            yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)parent->data)->file_index, YS_ERROR);  
            
            return "0";
        }else{
            return ((yamlsettings_node_element *)variable_children->data)->data;
        }

    }
    
    tmp=yamlsettings_parameter_name_normalized(parent);
    sprintf(error,"The variable %s used by a function (%s) was not found. It is used in the node %s",
            variable_name,
            value_read,
        tmp);
    free(tmp);
...
void yamlsettings_expand_functions(yamlsettings_find_usages_struct *data_find_usages, yamlsettings_status *status, int function /*1=expand; 2=range*/, GNode *variables){
    int i,j;
    int min, max, times;
    float repeated_value;
    yamlsettings_node_element *node_data;
    GNode *to_be_replaced, *parent, *variable_children;
    char copy_name[1000], *name, *value1, *value2, *char_ptr;
    int result;
    char error[512];
    ...
    for(i=0; i<data_find_usages->nuses; i++){
        ...
        name=((yamlsettings_node_element *)to_be_replaced->data)->data;
        
        strcpy(copy_name,name);
        char_ptr=strchr(copy_name, '(');
        value1=char_ptr+1;
        char_ptr=strchr(value1, ';');
        if (char_ptr==NULL){
            sprintf(error,"The function definition is wrong in the parameter %s. Values could not be extracted from '%s' (while looking for the symbol ';'). Please, check and define the function without any space or any other symbol out of those in the definition", 
                    ((yamlsettings_node_element *)parent->data)->data,
                    name);
            yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)parent->data)->file_index, YS_ERROR);  
            continue;
        ...
        *char_ptr='\0';
        if (char_ptr-copy_name!=strlen(name)-1){
            sprintf(error,"The function definition is wrong in the parameter %s. Values could not be extracted from '%s' (after stracting the values the string is still longer). Please, check and define the function without any space or any other symbol out of those in the definition", 
                    ((yamlsettings_node_element *)parent->data)->data,
                    name);
            yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)parent->data)->file_index, YS_ERROR);  
            continue;
        ...
        if(value1[0]=='^'){
            value1=yamlsettings_get_variable_value(variables,value1,parent,name,status);
        }
        if(value2[0]=='^'){
            value2=yamlsettings_get_variable_value(variables,value2,parent,name,status);
        }
        ...
        if (value1!=NULL && value2!=NULL){
            ...
            switch(function){
                case 1:
                    result+=sscanf(value1, "%d",&min);
                    result+=sscanf(value2, "%d",&max);
                    break;
                case 2:
                    result+=sscanf(value1, "%f",&repeated_value);
                    result+=sscanf(value2, "%d",&times);
                    break;
                default:
                    assert(0==1);
            }
            ...
            if (result!=2){
                sprintf(error,"The function definition is wrong in the parameter %s. Values could not be extracted from %s. Please, check and define the function without any space or any other symbol out of those in the definition", 
                        ((yamlsettings_node_element *)parent->data)->data,
                        name);
                yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)parent->data)->file_index, YS_ERROR);  
            }
            if(function==1){
                if (min > max){
                    sprintf(error,"The function definition %s is wrong in the parameter %s. Minimum value (%d) is greater than maximum value (%d)", 
                            name,
                            ((yamlsettings_node_element *)parent->data)->data,
                            min,max);
                    yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)parent->data)->file_index, YS_ERROR);  
            ...
            switch(function){
                case 1:
                    ((yamlsettings_node_element *)parent->data)->nelements += - 1 + max - min + 1;

                    for(j=max; j>=min; j--){
                        node_data=(yamlsettings_node_element *)malloc(sizeof(yamlsettings_node_element));
                        node_data->type=SEQ; 
                        node_data->nelements=0;
                        node_data->element_position=((yamlsettings_node_element *)to_be_replaced->data)->element_position;
                        node_data->file_index=((yamlsettings_node_element *)to_be_replaced->data)->file_index;
                        node_data->data=(char *)malloc(sizeof(char)*10);
                        sprintf(node_data->data,"%d", j);

                        g_node_insert_data(parent, ((yamlsettings_node_element *)to_be_replaced->data)->element_position, (gpointer)node_data);
                    }
                    // Fix index of positions
                    variable_children=parent->children;
                    j=0;
                    while(variable_children!=NULL){
                        ((yamlsettings_node_element *)variable_children->data)->element_position=j;
                        ((yamlsettings_node_element *)variable_children->data)->type=SEQ;
                        j++;
                        variable_children=variable_children->next;
                    }  
                ...
                case 2: 
                    ((yamlsettings_node_element *)parent->data)->nelements += - 1 + times;
                    ...
                    for(j=0; j<times; j++){
                        ...
                        node_data->element_position=((yamlsettings_node_element *)to_be_replaced->data)->element_position;
                        node_data->file_index=((yamlsettings_node_element *)to_be_replaced->data)->file_index;
                        node_data->data=(char *)malloc(sizeof(char)*20);
                        sprintf(node_data->data,"%g", repeated_value);
...
gboolean yamlsettings_variable_check_exist(GNode *node, gpointer data){
    yamlsettings_find_replace_remove_variables_struct *indata;
    yamlsettings_status *status;
    char error[512];
    char *name;
    ...
    if (yamlsettings_is_variable(name)){
        sprintf(error,"Variable %s is not defined", name);
        yamlsettings_error_add_parse_error(status, error, ((yamlsettings_node_element *)node->data)->file_index, YS_ERROR);
    }
...
void handle_variables(yamlsettings_status *status, GNode *root){
    yamlsettings_find_replace_remove_variables_struct data;
    GNode *variables;
    yamlsettings_node_element *dummy_data;
    yamlsettings_find_usages_struct data_find_usages;
        
    // Extract all variables to a new node. Creating the new node and preparing the call
    dummy_data = (yamlsettings_node_element *)malloc(sizeof(yamlsettings_node_element));
    dummy_data->data=(char *)malloc(sizeof(char)*5);
    strcpy(dummy_data->data,"");
    variables = g_node_new((gpointer)dummy_data);
    ...
    g_node_children_foreach (root, G_TRAVERSE_ALL, yamlsettings_find_replace_remove_variables, (gpointer)(&data));
     
    // Extracting and deal with functions
    data_find_usages.nuses=0;
    data_find_usages.searched_name="^expand(";
    data_find_usages.nchars=8;
    g_node_traverse(root, G_LEVEL_ORDER, G_TRAVERSE_LEAVES, -1, yamlsettings_find_usages, (gpointer)&data_find_usages);
    yamlsettings_expand_functions(&data_find_usages, status, 1, variables);
    
    data_find_usages.nuses=0;
    data_find_usages.searched_name="^repeat(";
    data_find_usages.nchars=8;
    g_node_traverse(root, G_LEVEL_ORDER, G_TRAVERSE_LEAVES, -1, yamlsettings_find_usages, (gpointer)&data_find_usages);
    yamlsettings_expand_functions(&data_find_usages, status, 2, variables);
    
    
    
    // Check if a variable is not defined
    g_node_traverse(root, G_LEVEL_ORDER, G_TRAVERSE_LEAVES, -1, yamlsettings_variable_check_exist, (gpointer)&data);
    
    // Remove variables
    yamlsettings_destroy_tree(variables);
}

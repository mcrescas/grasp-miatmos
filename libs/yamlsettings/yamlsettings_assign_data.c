            ...
            switch(param->allow_array){
                case YS_PARAM_SCALAR:
                    for(j=0;j<param->counter_mem_pos[i];j++){
                        ...
                        free(tmpchar);
                        g_string_append(gresult, ": ");    
                        tmpchar=yamlsettings_data_type_get_string(param->data_type_get, param->mem_pos, indexND(param->dimensions.ndimensions, indexes ,param->dimensions.dimension_size)/*0*/, param->maxlength, false);
                        g_string_append(gresult, tmpchar);
                        free(tmpchar);
                    }
                    break;
                case YS_PARAM_OPTIONAL_ARRAY:
                    ...
                    if(tmp>1){
                        ...
                        for(j=0;j<tmp;j++){
                            ...
                            tmpchar=yamlsettings_data_type_get_string(param->data_type_get, param->mem_pos, indexND(param->dimensions.ndimensions, indexes ,param->dimensions.dimension_size), param->maxlength, false);
                            g_string_append_printf(gresult," %s", tmpchar);
                            free(tmpchar);
                        }
                        g_string_append_printf(gresult," ] #%d", tmp);
                    }else{
                        indexes[yamlsettings_parameter_counter_number_of_dimensions(param)]=0;
                        tmpchar=yamlsettings_data_type_get_string(param->data_type_get, param->mem_pos, indexND(param->dimensions.ndimensions, indexes ,param->dimensions.dimension_size), param->maxlength, false);
                        g_string_append(gresult, tmpchar);
                        free(tmpchar);
                    }
